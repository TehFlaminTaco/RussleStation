...
/obj/mecha/verb/toggle_mining_scanner()
	...
	if(usr!=src.occupant)
		return
	if(src.equipment)
		var/obj/item/mecha_parts/mecha_equipment/tool/mining_scanner/T = locate() in src.equipment
		if(T)
			if(!T.pr_mining_scanner.active())
				T.pr_mining_scanner.start()
				src.occupant_message("Mining Scanner Activated.")
			else
				T.pr_mining_scanner.stop()
				src.occupant_message("Mining Scanner Deactivated.")

		...
		if(do_after_cooldown(target))
			if(T == chassis.loc && src == chassis.selected)
				if(istype(target, /turf/simulated/wall/r_wall))
					...
					log_message("Drilled through [target]")
					target.add_lastattacked(chassis.occupant)
					target.ex_act(2)
		return 1
...
/datum/global_iterator/mining_scanner

	process(var/obj/item/mecha_parts/mecha_equipment/tool/mining_scanner/RD as obj)
		if(!RD.chassis.occupant||!RD.chassis.occupant.client)
			stop()
			RD.set_ready_state(1)
			return
		else
			RD.set_ready_state(1)
			var/D = RD.chassis.loc
			var/client/C = RD.chassis.occupant.client
			var/list/L = list()
...
/obj/item/mecha_parts/mecha_equipment/tool/mining_scanner
	name = "Mech Mining Scanner"
	desc = "Costantly scans for ore in the surrounding area."
	icon = 'icons/obj/device.dmi'
	icon_state = "mmining"
	item_state = "analyzer"
	var/datum/global_iterator/pr_mining_scanner
	energy_drain = 10
	construction_cost = list("metal"=10000, "glass"=5000)
	...
	attach(obj/mecha/M as obj)
		..()
		pr_mining_scanner = new /datum/global_iterator/mining_scanner(list(src),0)
		chassis.verbs += /obj/mecha/verb/toggle_mining_scanner
		return
	detach()
		chassis.verbs -= /obj/mecha/verb/toggle_mining_scanner
		pr_mining_scanner.stop()
		pr_mining_scanner = null
		..()
		return
	action(atom/target)
		if(!action_checks(target)) return
		...
		if(istype(target,/turf/simulated/mineral))
			var/turf/simulated/mineral/R = target
			if(istype(R.mineral, /mineral/gibtonite))
				usr.visible_message("<span class='notice'>You use the mining scanner to locate where to cut off the chain reaction and attempt to stop it...</span>")
				R.defuse()

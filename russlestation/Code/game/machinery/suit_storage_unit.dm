...
/obj/machinery/suit_storage_unit/attackby(obj/item/I as obj, mob/user as mob)
	...
	if ( istype(I, /obj/item/weapon/grab) )
		...
		if(do_after(user, 20))
			...
			return
		return
	if ( istype(I, /obj/item/borg/grab) )
		var/obj/item/borg/grab/BG = I
		if( !(ismob(BG.attack)) )
			return
		if (!src.isopen)
			usr << "<font color='red'>The unit's doors are shut.</font>"
		...
		if ( (src.OCCUPANT) || (src.HELMET) || (src.SUIT) ) //Unit needs to be absolutely empty
			user << "<font color='red'>The unit's storage area is too cluttered.</font>"
			return
		visible_message("[user] starts putting [BG.attack.name] into the Suit Storage Unit.", 3)
		if(do_after(user, 20))
			if(!BG || !BG.attack) return //derpcheck
			var/mob/M = BG.attack
			if (M.client)
				M.client.perspective = EYE_PERSPECTIVE
				M.client.eye = src

	...
	switch(src.state)
		...
		if(STATE_ALERT_LEVEL)
			...
			if(security_level == SEC_LEVEL_GAMMA)
				dat += "<font color='red'><b>This is Centcom, we have dectected a rogue nuclear warhead that has appeared from a micro-wormhole. It is on its way and will impact the station in 30 seconds, by the time you read this message you are probably already dead, it was nice knowing you...</b></font>"
			else
...
/proc/init_shift_change(var/mob/user, var/force = 0)
	...
	if(!force)
		...
		if(sent_strike_team == 1 && admin_shuttle_call != 1)
			user << "Centcom will not allow the shuttle to be called. Consider all contracts terminated."
			return

		if(world.time < 54000 && admin_shuttle_call != 1) // 30 minute grace period to let the game get going
		...
		if(ticker.mode.name == "revolution" || ticker.mode.name == "AI malfunction" || ticker.mode.name == "sandbox" && admin_shuttle_call != 1)
			//New version pretends to call the shuttle but cause the shuttle to return after a random duration.
			emergency_shuttle.fake_recall = rand(300,500)

		if(ticker.mode.name == "blob" || ticker.mode.name == "epidemic" && admin_shuttle_call != 1)
			user << "Under directive 7-10, [station_name()] is quarantined until further notice."
			return
	world << sound('sound/AI/shuttletranscalled.ogg')
...
/proc/cancel_call_proc(var/mob/user)
	...
	if((ticker.mode.name == "blob")||(ticker.mode.name == "meteor"))
		return
	if(admin_shuttle_call == 1)
		usr << "\red <b>The shuttle recall button seems to be stuck!</b>"
		return

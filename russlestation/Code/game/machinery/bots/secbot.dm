...
/obj/item/clothing/head/helmet/attackby(var/obj/item/device/B, mob/user as mob)
	..()
	if(!issignaler(B))
		..()
		return
	var/obj/item/device/assembly/signaler/S = B

	if(src.type != /obj/item/clothing/head/helmet) //Eh, but we don't want people making secbots out of space helmets.
		return
	for(var/obj/item/device/paicard/D in src)
		if(D)
			user << "you cannot add this while a pAI is inside the helmet!"
			return

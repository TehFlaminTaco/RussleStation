	...
	if(!issilicon(usr))
		if(src.isElectrified())
			if(!src.justzap)
				...
				return
		else if(user.hallucination > 50 && prob(10) && src.operating == 0 && !(user.flags & IS_SYNTHETIC))
...
/obj/machinery/door/airlock/attackby(C as obj, mob/user as mob)
	...
	if((istype(C, /obj/item/weapon/weldingtool) && !( src.operating > 0 ) && src.density))
	...
	else if(istype(C, /obj/item/device/paicard))
		var/obj/item/device/paicard/pai = C
		if(pai.pai && pai.pai.cable && (pai.pai.cable.loc == src))
			for(var/obj/item/weapon/pai_cable/T in src.contents)
				if(T)
					if(T == pai.pai.cable)
						del(T)
						var/turf/D = get_turf_or_move(src.loc)
						for (var/mob/M in viewers(D))
							M.show_message("\red [pai.pai.cable] rapidly retracts back into its spool.", 3, "\red You hear a click and the sound of wire spooling rapidly.", 2)
		...
		if( beingcrowbarred && (operating == -1 || density && welded && operating != 1 && src.p_open && (!src.arePowerSystemsOn() || stat & NOPOWER) && !src.locked) )
			...
			if(do_after(user,40))
				...
				da.anchored = 1
				var/turf/simulated/location = get_turf(da)
				if(location.zone)
					for(var/turf/T in location.zone.contents)
						for(var/obj/item/tape/engineering/F in T.contents)
							del(F)
						for(var/obj/item/tape/police/P in T.contents)
							del(P)

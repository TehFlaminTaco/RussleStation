...
/obj/machinery/door/firedoor/New()
	...
	for(var/direction in cardinal)
		...
		if(istype(A) && !(A in areas_added))
			...
			areas_added += A
	if(density)
		layer = 2.7 //Above most items if closed
...
/obj/machinery/door/firedoor/Bumped(atom/AM)
	...
	if(istype(AM, /obj/mecha))
		...
		if (mecha.occupant)
			...
			attack_hand(M)
	if(istype(AM, /obj/structure/stool/bed/chair/cart))
		var/obj/structure/stool/bed/chair/cart/C = AM
		if (C.buckled_mob)
			var/mob/M = C.buckled_mob
			if(world.time - M.last_bumped <= 10) return //Can bump-open one airlock per second. This is to prevent popup message spam.
			M.last_bumped = world.time
			attack_hand(M)
	for(var/obj/machinery/door/D in src.loc.contents)
...
/obj/machinery/door/firedoor/attack_hand(mob/user as mob)
	...
	var/answer = alert(user, "Would you like to [density ? "open" : "close"] [src.name]?[ alarmed && density ? "\nNote that by doing so, you acknowledge any damages from opening [src.name] as being your own fault, and you will be held accountable under the law." : ""]",\

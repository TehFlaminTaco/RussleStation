...
/obj/machinery/atmospherics/unary/cryo_cell/attackby(var/obj/item/G as obj, var/mob/user as mob)
	...
	if(istype(G, /obj/item/weapon/grab))
		var/obj/item/weapon/grab/HG = G
		if(!ismob(HG.affecting))
			return
		for(var/mob/living/carbon/slime/M in range(1,HG.affecting))
			if(M.Victim == HG.affecting)
				usr << "[HG.affecting:name] will not fit into the cryo because they have a slime latched onto their head."
				return
		var/mob/M = HG.affecting
		if(put_mob(M))
			del(HG)
			...
			if (istype(BG.attack, /mob/living))
				...
				for(var/mob/living/carbon/slime/M in range(1,BG.attack))
					if(M.Victim == BG.attack)
						usr << "[BG.attack:name] will not fit into the cryo because they have a slime latched onto their head."
						return
				var/mob/M = BG.attack
				if(put_mob(M))
					BG.attack = null

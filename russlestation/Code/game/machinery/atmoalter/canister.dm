...
/obj/machinery/portable_atmospherics/canister/Topic(href, href_list)
	...
	if(href_list["toggle"])
		if (valve_open)
			...
			if(alert("Are you sure you want to open the canister it could flood the area if there is no tank or it is not connected?",,"Yes","No")=="No")
				return
			if (holding)
	...
	if (href_list["remove_tank"])
		if(holding)
			if(alert("Are you sure you want remove this tank it could flood the area if the valve is open or is not connected?",,"Yes","No")=="No")
				return
			...
			holding = null
			if(valve_open)
				message_admins("<font color='#386aff'>[usr.name] ([usr.ckey]) took out a tank and opened a canister into [usr.lastarea]</font>(<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[src.x];Y=[src.y];Z=[src.z]'>JMP</a>)(<a href='?src=\ref[usr];priv_msg=\ref[usr]'>PM</a>)" )

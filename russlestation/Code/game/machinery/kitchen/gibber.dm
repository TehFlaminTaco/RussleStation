...
/obj/machinery/gibber/attackby(obj/item/G as obj, mob/user as mob)
	if(src.occupant)
		user << "\red The gibber is full, empty it first!"
		return

	if( !(istype(G, /obj/item/weapon/grab)) && !(istype(G, /obj/item/borg/grab)) )
	...
	if(istype(G, /obj/item/weapon/grab))
		...
		if(do_after(user, 30) && G && G:affecting && !occupant)
			...
			del(G)
			update_icon()
	if(istype(G, /obj/item/borg/grab))
		if(!(istype(G:attack, /mob/living/carbon)) && !(istype(G:attack, /mob/living/simple_animal)))
			user << "\red This item is not suitable for the gibber!"
			return
		if(G:attack.abiotic(1))
			user << "\red Subject may not have abiotic items on."
			return

		user.visible_message("\red [user] starts to put [G:attack] into the gibber!")
		src.add_fingerprint(user)
		if(do_after(user, 30) && G && G:attack && !occupant)
			user.visible_message("\red [user] stuffs [G:attack] into the gibber!")
			var/mob/M = G:attack
			if(M.client)
				M.client.perspective = EYE_PERSPECTIVE
				M.client.eye = src
	...
	if( istype(src.occupant, /mob/living/carbon/human/) )
		...
		for(var/i=1 to totalslabs)
			if(H.species.flags & IS_SYNTHETIC)
				var/obj/item/weapon/reagent_containers/food/snacks/meat/human/syntiflesh/newmeat = new

	...
	if(istype(traitor.current, /mob/living/silicon))
		...
		switch(rand(1,100))
			if(1 to 39)
				...
				traitor.objectives += kill_objective
			if(40 to 50)
				var/datum/objective/stealmoney/steal_money = new
				steal_money.owner = traitor
				steal_money.gen_amount_goal()
				traitor.objectives += steal_money
			if(51 to 66)
...
/datum/game_mode/proc/greet_traitor(var/datum/mind/traitor)
	...
	traitor.current.faction = "syndicate"
	if (!config.objectives_disabled)
		var/obj_count = 1
	...
	if(traitors.len)
		...
		for(var/datum/mind/traitor in traitors)
			...
			if(traitor.key)
				...
				text += "<br>Traitor(DC) was [traitor.name] ("
			if(traitor.current)

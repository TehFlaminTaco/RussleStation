...
/obj/item/clothing/suit/space/space_ninja/New()
	...
	cell.charge = 12500//Starting charge should not be higher than maximum charge. It leads to problems with recharging.
		...
		for(var/i = 0,i<7,i++)
			switch(i)
				...
				if(6)
					...
					remove_equip_verbs()
					unlock_suit()
					if(U.hud_used.ninja_cell_display)
						U.client.screen -= U.hud_used.ninja_cell_display
						U.hud_used.ninja_cell_display = null
					U.hud_used.instantiate()
					if(U.hud_used.vampire_blood_display)
						U.hud_used.vampire_hud()
					if(U.hud_used.changeling_chem_display)
						U.hud_used.changeling_hud()
					U.regenerate_icons()
			...
			if(A.charge)
				if (G.candrain&&do_after(U,30))
					...
					if(S.cell.charge+A.charge>S.cell.maxcharge)
						A.charge = A.charge-(S.cell.maxcharge-S.cell.charge)
						U << "\blue You fully charge your suit with <B>[S.cell.maxcharge-S.cell.charge]</B> energy from the cell."
	...
	for(var/mob/living/target in target_list)
		if(iscarbon(target))
			...
			if(istype(target,/mob/living/simple_animal/borer))
				U.client.images += image(tempHud,target,"hudbrainworm2")
			else
				U.client.images += image(tempHud,target,"hudunknown1")
	return 1
...
/obj/item/clothing/mask/gas/voice/space_ninja/proc/switchm()
	...
	switch(mode)
		...
		if(1)
			mode=2
			U.see_in_dark = 7
			U.client.screen -= global_hud.nvg
			U << "Switching mode to <B>Thermal Scanner</B>."
		if(2)
			mode=3
			U.sight &= ~SEE_MOBS
			for(var/obj/item/clothing/glasses/thermal/G in U.contents)
				del(G)

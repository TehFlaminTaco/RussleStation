...
/proc/create_blackhole(var/turf/enter as turf)
	if(blackholes == 1)
		return
	else
		if(prob(1))
			var/obj/effect/bhole/bh = new /obj/effect/bhole( enter )
			blackholes = 1
			message_admins("blackhole spawned (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[enter.x];Y=[enter.y];Z=[enter.z]'>JMP</a>)")
			spawn(rand(100, 600))
				del(bh)
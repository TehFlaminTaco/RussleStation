...
mob/living/parasite/meme/emote(message as text)
	if(dormant)
		usr << "\red You're dormant!"
		return
...
// --------------------------------------------
mob/living/parasite/get_contents(t)
	if(!host) return 0

	return host.get_contents(t)

/*mob/living/parasite/has_selected_beaker_reagents(t)
	if(!host) return 0

	return host.has_selected_beaker_reagents(t)*/
...
/datum/game_mode/meme/can_start()
	...
	recommended_enemies = max(src.num_players() / 30 * 2, 2)

	var/list/datum/mind/possible_memes = get_players_for_role(BE_ALIEN)
...
/datum/game_mode/meme/post_setup()
	var/memenames = list("Ji", "Ka", "La", "Kak", "Ma", "Da", "Li", "Ti", "Sa")
	...
	for(var/datum/mind/meme in memes)
		...
		meme.transfer_to(M)
		M.clearHUD()
		M.name = "[pick(memenames)]'[pick(memenames)]'[pick(memenames)]"

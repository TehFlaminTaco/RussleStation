...
//					player.unlock_medal("Guardin' gnome", 1)
	...
	for(var/mob/living/carbon/human/E in mob_list)
		...
		if(E.stat != 2 && location in escape_zone) // Escapee Scores
			if(E.mind.initial_account)
				cashscore += E.mind.initial_account.money
			for (var/obj/item/weapon/spacecash/C2 in E.contents) cashscore += C2.worth
			for (var/obj/item/weapon/coin/C3 in E.contents) cashscore += C3.credits
			for (var/obj/item/weapon/storage/S in E.contents)
				for (var/obj/item/weapon/spacecash/C5 in S.contents) cashscore += C5.worth
				for (var/obj/item/weapon/coin/C6 in S.contents) cashscore += C6.credits
...
//				if(Ba.fields["name"] == E.real_name) cashscore += Ba.fields["current_money"]
			...
			else if (cashscore < score_richestcash &&(cashscore !=0) && (cashscore < score_poorlastcheck))
				score_poorlastcheck = cashscore
				score_poorestcash = cashscore
				score_poorestname = E.real_name
				score_poorestjob = E.job
				score_poorestkey = E.key
...
//		if (istype(M, /obj/decal/cleanable/urine)) score_mess += 1
	...
	var/harvests = score_stuffharvested * 2 //done
	var/shipping = score_stuffshipped * 5
	var/coins = score_coins * 2
	var/mining = score_oremined  *0.5//done
	var/meals = score_meals * 5 //done, but this only counts cooked meals, not drinks served
	var/tdeaths = score_total_deaths * 25
	var/ccloned = score_total_cloned * 25
	var/podman = score_total_podman * 25
	var/power = score_powerloss * 20
	var/suicide = score_suicide * 25
	var/messpoints
	...
	if (ticker.mode.config_tag == "nuclear")
		...
		if (score_nuked)
			score_crewscore -= nukedpenalty
		else if(!score_nuked)
			score_crewscore += nukedpenaltygood
	...
	score_crewscore += coins
	score_crewscore += researchpoints
	score_crewscore += eventpoints
	score_crewscore += escapoints
	score_crewscore += ccloned
	score_crewscore += podman
	if(adminpointsgood)
		for(var/i in adminscoregoodtitle)
			score_crewscore += text2num(adminscoregoodtitle[i])
	...
	score_crewscore -= tdeaths
	score_crewscore -= suicide
	if(adminpointsbad)
		for(var/i in adminscorebadtitle)
			score_crewscore -= text2num(adminscorebadtitle[i])
...
/mob/proc/scorestats()
	...
	if (ticker.mode.name == "nuclear emergency")
		...
		<B>Station Destroyed:</B> [score_nuked ? "Yes" : "No"] ([score_nuked ? "-" : ""][score_nuked ? "[nukedpenalty]" : "[nukedpenaltygood]"] Points)<BR>
...
//	var/totalfunds = wagesystem.station_budget + wagesystem.research_budget + wagesystem.shipping_budget
	...
	<U>THE GOOD:</U><BR>
	<B>Items Shipped:</B> [score_stuffshipped] ([score_stuffshipped * 5] Points)<BR>
	<B>Hydroponics Harvests:</B> [score_stuffharvested] ([score_stuffharvested * 2] Points)<BR>
	<B>Ore Mined:</B> [score_oremined] ([score_oremined*0.5] Points)<BR>
	<B>Coins Pressed:</B> [score_coins] ([score_coins * 2] Points)<BR>
	<B>Meals Prepared:</B> [score_meals] ([score_meals * 5] Points)<BR>
	...
	<B>Ultra-Clean Station:</B> [score_mess ? "No" : "Yes"] ([score_messbonus * 3000] Points)<BR>
	<B>Total Cloned Players:</B> [score_total_cloned] ([score_total_cloned * 25] Points)<BR>
	<B>Total Diona Grown:</B> [score_total_podman] ([score_total_podman * 25] Points)<BR><BR>
	<U>THE BAD:</U><BR>
	<B>Dead Bodies on Station:</B> [score_deadcrew] (-[score_deadcrew * 25] Points)<BR>
	<B>Total Deaths:</B> [score_total_deaths] (-[score_total_deaths *25] Points)<BR>
	...
	<B>AI Destroyed:</B> [score_deadaipenalty ? "Yes" : "No"] (-[score_deadaipenalty * 250] Points)<BR>
	<B>Number of crew suicides or cryogenic freeezes:</B> [score_suicide] (-[score_suicide * 25] Points)<BR><BR>
	<U>THE WEIRD</U><BR>"}
/*	<B>Final Station Budget:</B> $[num2text(totalfunds,50)]<BR>"}
	...
	<B>Drinks Consumed:</b> [score_drinksconsumbed]<BR>
	<B>Times a Clown was Abused:</B> [score_clownabuse]<BR>
	<B>Number of Times Players slipped:</B> [score_slipped]<BR>
	<B>Number of units of Space Drugs consumed:</B> [space_drugs_consumed]<BR><BR>"}
	if(adminpointsgood || adminpointsbad)
		dat += {"<U>THE UGLY</U><BR>"}
	if(adminpointsgood ==1)
		for(var/i in adminscoregoodtitle)
			dat += {"<B>[i]:</b> [adminscoregoodtitle[i]] Points<BR>"}
	if(adminpointsbad ==1)
		for(var/i in adminscorebadtitle)
			dat += {"<B>[i]:</b> -[adminscorebadtitle[i]] Points<BR>"}



	if (score_escapees)
		dat += {"<B>Richest Shuttle Escapee:</B> [score_richestname], [score_richestjob]: $[num2text(score_richestcash,50)] ([score_richestkey])<BR>
		<B>Most Battered Shuttle Escapee:</B> [score_dmgestname], [score_dmgestjob]: [score_dmgestdamage] damage ([score_dmgestkey])<BR>"}
	else

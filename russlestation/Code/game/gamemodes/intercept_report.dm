...
/datum/intercept_text/proc/build(var/mode_type, datum/mind/correct_person)
	switch(mode_type)
		...
		if("assimilate")
			src.text = ""
			src.build_assim(correct_person)
			return src.text
		else
			return null
...
/datum/intercept_text/proc/build_assim(datum/mind/correct_person)
	src.text += "<BR><BR>Cent. Com recently recieved a report of a shuttle aproaching your area. We believe it to be of no harmful intent."
	src.text += "We recommend ignoring it unless it become a threat "
	src.text += "As this may cause panic among the crew, all efforts should be made to keep this information a secret from all but "
	src.text += "the most trusted crew-members."

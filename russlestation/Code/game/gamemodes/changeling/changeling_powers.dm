...
/mob/proc/changeling_absorb_dna()
	...
	T.death(0)
	T.Drain()
	T.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been absrobed by [usr.name] ([usr.ckey])</font>")
	usr.attack_log += text("\[[time_stamp()]\] <font color='red'>Used absrobed [T.name] ([T.ckey])</font>")
	log_attack("[usr.name] ([usr.ckey]) absorbed [T.name] ([T.ckey])")
	msg_admin_attack("[key_name(usr)] absorbed [T.name] ([T.ckey]).")
	return 1
...
/mob/proc/changeling_sting(var/required_chems=0, var/verb_path)
	if(no_attack ==1)
		src << "\red attacks have been disabled"
		return
	if(src.stat || src.stunned || src.weakened || src.paralysis)
		src << "<span class='warning'>We are incapacitated.</span>"
		return
...
/mob/proc/changeling_lsdsting()
	...
	var/powername = "Hallucination Sting"
...
/mob/proc/changeling_transformation_sting()
	...
	set desc="Sting target"
	var/powername = "Transformation sting"
	...
	if(!T)	return 0
	if((HUSK in T.mutations) || (!ishuman(T) && !ismonkey(T))|| (H.species.flags & IS_SYNTHETIC))
...
/mob/proc/changeling_extract_dna_sting()
	...
	var/powername = "Extract DNA Sting"

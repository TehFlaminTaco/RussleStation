...
/datum/game_mode
	...
	var/list/restricted_species = list() //species that can't be traitors becuase
...
/obj/item/toy/cards/deck/syndicate:2:Syndicate Playing Cards;
Whitespace:Seperator;
...
/datum/game_mode/proc/declare_completion()
	...
	var/escaped_on_pod_med = 0
	var/escaped_on_arrival = 0
	var/escaped_on_shuttle = 0

	var/list/area/escape_locations = list(/area/shuttle/escape/centcom, /area/shuttle/escape_pod1/centcom, /area/shuttle/escape_pod2/centcom, /area/shuttle/escape_pod3/centcom, /area/shuttle/escape_pod4/centcom, /area/shuttle/escape_pod5/centcom, /area/shuttle/escape_pod6/centcom, /area/shuttle/escape_pod7/centcom, /area/shuttle/escape_pod8/centcom, /area/shuttle/escape_pod_medical/centcom, /area/shuttle/arrival/centcom)
		...
		if(M.client)
			...
			if(!M.stat)
				...
				if(M.loc && M.loc.loc && M.loc.loc.type == /area/shuttle/escape_pod5/centcom)
					escaped_on_pod_5++
				if(M.loc && M.loc.loc && M.loc.loc.type == /area/shuttle/escape_pod6/centcom)
					escaped_on_pod_6++
				if(M.loc && M.loc.loc && M.loc.loc.type == /area/shuttle/escape_pod7/centcom)
					escaped_on_pod_7++
				if(M.loc && M.loc.loc && M.loc.loc.type == /area/shuttle/escape_pod8/centcom)
					escaped_on_pod_8++
				if(M.loc && M.loc.loc && M.loc.loc.type == /area/shuttle/escape_pod_medical/centcom)
					escaped_on_pod_med++
				if(M.loc && M.loc.loc && M.loc.loc.type == /area/shuttle/arrival/centcom)
					escaped_on_arrival++
	...
	if(escaped_on_pod_7 > 0)
		feedback_set("escaped_on_pod_7",escaped_on_pod_7)
	if(escaped_on_pod_8 > 0)
		feedback_set("escaped_on_pod_8",escaped_on_pod_8)
	if(escaped_on_pod_med > 0)
		feedback_set("escaped_on_pod_med",escaped_on_pod_med)
	if(escaped_on_arrival > 0)
		feedback_set("escaped_on_arrival",escaped_on_arrival)
...
/datum/game_mode/proc/send_intercept()
	...
	for(var/mob/living/carbon/human/man in player_list) if(man.client && man.mind)
		...
		if (special_role == "Wizard" || special_role == "Ninja" || special_role == "Syndicate"|| special_role == "Vox Raider"|| special_role == "Syndicate Assimilation Borg")
	...
	if(restricted_species)
		for(var/datum/mind/player in candidates)
			for(var/species in restricted_species)
				if(player.checkspecies == species)
					candidates -= player
...
proc/display_roundstart_logout_report()
	...
	for(var/mob/living/L in mob_list)
		...
		if(L.ckey)
			...
			if(!found)
				if(L.aghost)
					msg += "<b>[L.name]</b> ([L.ckey]), the [L.job] (Admin ghosted)\n"
				else

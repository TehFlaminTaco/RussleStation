...
/mob/proc/vampire_power(required_blood=0, max_stat=0)
	...
	if(!ishuman(src))
		...
		return 0
	if(no_attack == 1)
		src << "\red Attacks have been disabled"
		return
...
/client/vampire/proc/vampire_hypnotise()
	...
	if(do_mob(M.current, C, 50))
		if(C.mind && C.mind.vampire)
			...
			C.Stun(20)
			C.stuttering = 20
			C.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has had [powername] used on them by [usr.name] ([usr.ckey])</font>")
			usr.attack_log += text("\[[time_stamp()]\] <font color='red'>Used [powername] on [C.name] ([C.ckey])</font>")
			log_attack("[usr.name] ([usr.ckey]) Used [powername] on [C.name] ([C.ckey])")
			msg_admin_attack("[key_name(usr)] Used [powername] on [C.name] ([C.ckey]).")
	else
...
/client/vampire/proc/vampire_screech()
	...
	var/powername = "Chiroptean Screech"
	var/datum/mind/M = usr.mind
	if(!M) return
...
/mob/proc/can_enthrall(mob/living/carbon/C)
	...
	if(!C.mind)
		...
		return 0
	if((/obj/item/weapon/implant/loyalty in C.contents )||( C.mind in ticker.mode.vampires )||( C.mind.vampire )||( C.mind in ticker.mode.enthralled ))
...
/client/vampire/proc/vampire_jaunt()
	...
	if(M.current.vampire_power(30, 0))
		...
		spawn(0)
			...
			animation.name = "blood_water"
			animation.density = 0
			animation.anchored = 1
			animation.icon = 'icons/mob/mob.dmi'
			animation.icon_state = "blood_liquify"
			animation.layer = 5
			animation.master = holder
			M.current.ExtinguishMob()
			if(M.current.buckled)
				M.current.buckled.unbuckle()
			flick("blood_liquify",animation)
			M.current.loc = holder
			M.current.client.eye = holder
			var/datum/effect/effect/system/blood_steam_spread/blood_steam = new /datum/effect/effect/system/blood_steam_spread()
			blood_steam.set_up(10, 0, originalloc)
			blood_steam.start()
			sleep(jaunt_duration)
			var/mobloc = get_turf(M.current.loc)
			if(get_area(mobloc) == /area/security/warden)
				M << "A strange energy repels you!"
				mobloc = originalloc
			animation.loc = mobloc
			blood_steam.location = mobloc
			blood_steam.start()
			M.current.canmove = 0
			sleep(20)
			flick("blood_reappear",animation)
			sleep(5)

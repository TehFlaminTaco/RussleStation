		...
		for(var/mob/living/player in player_list)
			...
			if(player.mind)
				if(player.stat != DEAD)
					...
					if(!T)	continue
					switch(T.loc.type)
						if(/area/shuttle/escape/centcom, /area/shuttle/escape_pod1/centcom, /area/shuttle/escape_pod2/centcom, /area/shuttle/escape_pod3/centcom, /area/shuttle/escape_pod4/centcom, /area/shuttle/escape_pod5/centcom, /area/shuttle/escape_pod6/centcom, /area/shuttle/escape_pod7/centcom, /area/shuttle/escape_pod8/centcom, /area/shuttle/escape_pod_medical/centcom, /area/shuttle/arrival/centcom)
							return 0
		return 1
...
datum/objective/stealmoney

	proc/gen_amount_goal()
		target_amount = rand(10000,35000)
		explanation_text = "Steal $[target_amount] from the station by the end of the shift.(Must be on you or in your account.)"
		return target_amount
	check_completion()
		if(!isliving(owner.current))	return 0
		var/list/all_items = owner.current.get_contents()
		var/amount = 0
		amount += owner.initial_account.money
		for (var/obj/item/weapon/spacecash/C2 in all_items) amount += C2.worth
		for (var/obj/item/weapon/coin/C3 in all_items) amount += C3.credits

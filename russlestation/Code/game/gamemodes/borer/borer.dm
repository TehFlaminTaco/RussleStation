...
/datum/game_mode/borer/can_start()
	...
	for(var/obj/machinery/atmospherics/unary/vent_pump/v in world)
		if(!v.welded && v.z == STATION_Z) // No more spawning in atmos.  Assuming the mappers did their jobs, anyway.
			found_vents.Add(v)
	for(var/datum/mind/player in possible_borers)
		for(var/job in restricted_jobs)//Removing robots from the list
			if(player.assigned_role == job)
				possible_borers -= player

	borer_amount = round((num_players() / 10 )*2)
	// for each 2 possible borers, add one borer and one host
	if(possible_borers.len>2)
		for(var/i = 0, i < borer_amount, i++)
			if(!possible_borers.len) break
...
/datum/game_mode/borer/post_setup()
	...
	for(var/datum/mind/borer in borers)
		...
		var/mob/living/simple_animal/borer/GM/M = new(spawn_loc) // loc, by_gamemode=0
		...
		first_host.current.hud_updateflag |= 1 << SPECIALROLE_HUD
...
/datum/game_mode/proc/greet_borer(var/datum/mind/borer, var/you_are=1)
	if (you_are)
		...
		borer.current << "<B>\blue Your Borer's True Name is [borer.current:truename]</B>"
...
/datum/game_mode/proc/auto_declare_completion_borer()
	if(borers.len)
		var/text = "<FONT size = 2><B>The borers were:</B></FONT><BR>"
		for(var/datum/mind/borer in borers)
			var/borerwin = 1
			if(borer.key)
				text += "<br>[borer.key] was [borer.current:truename] ("
			else
				text += "<br>borer(DCed) was [borer.current:truename] ("
			if(borer.current)
				if(borer.current.stat == DEAD)
					...
					text += "survived)"
				if(istype(borer.current,/mob/living/simple_animal/borer))
					var/mob/living/simple_animal/borer/B = borer.current
					if(B.host)
						if(B.host.name && B.host.key)
							if(!B.controlling)
								text += "<br>The last host was [B.host.name]([B.host.key])<br>"
							else
								if(B.host_brain.key)
									text += "<br>The last host was [B.host.name]([B.host_brain.key])<br>"
								else
									text += "<br>The last host was [B.host.name](DCed)<br>"
						else
							text += "<br>The last host was [B.host.name](DCed)<br>"
					else
						text += "<br>"
					...
					if(borer.objectives.len)
						...
						for(var/datum/objective/objective in borer.objectives)
							if(objective.check_completion())
								...
								text += "<B>Objective #[count]</B>: [objective.explanation_text] <font color = red><B>Failed</b></font><br>"
			...
			if(borerwin)
				text += "<B><font color = green>The borer was successful!</font></B><br><br>"
				feedback_add_details("borer_success","SUCCESS")
			else
				text += "<B><font color = red>The borer has failed!</font></B><br><br>"

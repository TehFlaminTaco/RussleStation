	...
	if(istype(I, /obj/item/weapon/wrench))
		...
		if(do_after(user, 50))
			switch(watertemp)
				...
				if("boiling")
					watertemp = "normal"
			user.visible_message("<span class='notice'>[user] adjusts \the [I].</span>", "<span class='notice'>You adjust the shower with \the [I].</span>")
...
/obj/machinery/shower/proc/check_heat(mob/M as mob)
	...
	if(iscarbon(M))
		...
		if(watertemp == "freezing")
			C.bodytemperature = max(150, C.bodytemperature - 80)
...
/obj/structure/sink/attackby(obj/item/O as obj, mob/user as mob)
	...
	if (istype(O, /obj/item/weapon/reagent_containers))
		...
		if(RG.volume != RG.reagents.total_volume)
			RG.reagents.add_reagent("water", min(RG.volume - RG.reagents.total_volume, RG.amount_per_transfer_from_this))
			user.visible_message("\blue [user] fills \the [RG] using \the [src].","\blue You fill \the [RG] with [RG.amount_per_transfer_from_this] units of water using \the [src].")
			return
		else
			user << "\red The [RG] is full!"

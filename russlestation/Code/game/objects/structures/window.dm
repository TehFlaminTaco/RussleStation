...
/obj/structure/window/bullet_act(var/obj/item/projectile/Proj)
	...
	if(health <= 0)
		...
		S.add_lastattacked(Proj.firer)
		var/obj/item/stack/rods/R = new /obj/item/stack/rods(loc)
		R.add_lastattacked(Proj.firer)
		del(src)
	return
...
/obj/structure/window/hitby(atom/movable/AM as mob|obj)
	..()
	if(istype(AM,/obj/))
		var/obj/O = AM

		visible_message("<span class='danger'>[src] was hit by [O.name].</span>")
		...
		if(assailant && assailant.mob && istype(assailant.mob,/mob))
			...
			if(health <= 0)
				var/obj/item/weapon/shard/S = new /obj/item/weapon/shard( src.loc )
				S.griefcheck += "\[[time_stamp()]\] Broken by [O.name] thrown by [M.real_name]([M.ckey])<BR><BR>"
				if(reinf)
					var/obj/item/stack/rods/R = new /obj/item/stack/rods(loc)
					R.griefcheck += "\[[time_stamp()]\] Broken by [O.name] thrown by [M.real_name]([M.ckey])<BR><BR>"
				del(src)
		...
		var/client/assailant = directory[O.lastattacker]
		if(assailant && assailant.mob && istype(assailant.mob,/mob))
			var/mob/M = assailant.mob
			...
			if(health <= 0)
				var/obj/item/weapon/shard/S = new /obj/item/weapon/shard( src.loc )
				S.griefcheck += "\[[time_stamp()]\] Broken by [O.real_name] who was thrown by [M.real_name]([M.ckey])<BR><BR>"
				if(reinf)
					var/obj/item/stack/rods/R = new /obj/item/stack/rods(loc)
					R.griefcheck += "\[[time_stamp()]\] Broken by [O.real_name] who was thrown by [M.real_name]([M.ckey])<BR><BR>"
				del(src)
...
/obj/structure/window/attack_hand(mob/user as mob)
	if(HULK in user.mutations)
		...
		var/obj/item/weapon/shard/S = new /obj/item/weapon/shard( src.loc )
		S.add_lastattacked(user)
		if(reinf)
			var/obj/item/stack/rods/R = new /obj/item/stack/rods(loc)
			R.add_lastattacked(user)
		del(src)
	...
	if (istype(W, /obj/item/weapon/grab) && get_dist(src,user)<2)
		...
		if (istype(G.affecting, /mob/living))
			if(Adjacent(G.affecting))
				...
				return
	if (istype(W, /obj/item/borg/grab) && get_dist(src,user)<2)
		var/obj/item/borg/grab/G = W
		if (istype(G.attack, /mob/living))
			if(Adjacent(G.attack))
				var/mob/living/M = G.attack
				G.attack = null
				user.stop_pulling()
				G.process()
				M.apply_damage(7)
...
/obj/structure/window/proc/hit(var/damage, var/sound_effect = 1)
	...
	if(health <= 0)
		if(dir == SOUTHWEST)
			...
			index = 0
			while(index < 2)
				var/obj/item/weapon/shard/S = new shardtype(loc)
				S.add_lastattacked(hitby)
				if(reinf)
					var/obj/item/stack/rods/R = new /obj/item/stack/rods(loc)
					R.add_lastattacked(hitby)
				index++
		else
			var/obj/item/weapon/shard/G = new shardtype(loc)
			G.add_lastattacked(hitby)
			if(reinf)
				var/obj/item/stack/rods/R = new /obj/item/stack/rods(loc)
				R.add_lastattacked(hitby)
		del(src)

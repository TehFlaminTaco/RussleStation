	...
	spawn(2) //So it properly updates when deleting
		...
		if(flipped)
			...
			if (istype(src, /obj/structure/table/reinforced))
				base = "rtable"
			if (istype(src, /obj/structure/table/cardtable))
				base = "card"
		...
		else if(istype(src,/obj/structure/table/cardtable))
			switch(table_type)
				if(0)
					icon_state = "card_table"
				if(1)
					icon_state = "card_1tileendtable"
				if(2)
					icon_state = "card_1tilethick"
				if(3)
					icon_state = "card_tabledir"
				if(4)
					icon_state = "card_middle"
				if(5)
					icon_state = "card_tabledir2"
				if(6)
					icon_state = "card_tabledir3"
		else
			switch(table_type)
				if(0)
					icon_state = "table"
...
/obj/structure/table/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/borg/grab)&& get_dist(src,user)<2)
		...
		if(G.attack && get_dist(src,G.attack)<2)
			if (istype(G.attack, /mob/living))
				G.attack.loc = src.loc
				G.attack.Weaken(5)
				visible_message("\red [user] puts [G.attack] on \the [src].")
				G.attack = null
				user.stop_pulling()
				G.process()
				return
...
/obj/structure/table/mintinput
	name = "Coin Press Input"
	desc = "Add stacks of minerals here for processing."
	icon_state = "mintinput"
	health = 50
	density = 1
	anchored = 1.0
	layer = 2.9
...
/obj/structure/table/mintinput/update_icon()
	return

/obj/structure/table/mintinput/update_adjacent()
	return

/obj/structure/table/mintinput/flip(var/direction)
	return
...
/obj/structure/kitchenspike
	...
	attackby(obj/item/G as obj, mob/user as mob)
		...
		if(istype(G, /obj/item/borg/grab))
			if(istype(G:attack, /mob/living/carbon/monkey))
				if(src.occupied == 0)
					src.icon_state = "spikebloody"
					src.occupied = 1
					src.meat = 5
					src.meattype = 1
					for(var/mob/O in viewers(src, null))
						O.show_message(text("\red [user] has forced [G:attack] onto the spike, killing them instantly!"))
					del(G:attack)
					G:attack = null
					user.stop_pulling()
					G:process()
					...
					user << "\red The spike already has something on it, finish collecting its meat first!"
			else if(istype(G:attack, /mob/living/carbon/alien))
				if(src.occupied == 0)
					src.icon_state = "spikebloodygreen"
					src.occupied = 1
					src.meat = 5

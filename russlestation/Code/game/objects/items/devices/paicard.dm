...
/obj/item/device/paicard/proc/setPersonality(mob/living/silicon/pai/personality)
	src.pai = personality
	verbs += /obj/item/device/paicard/proc/pai_holo
	processing_objects += src
...
/obj/item/device/paicard/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/weapon/pai_cable))
		if(pai)
			if(pai.cable && (pai.cable == I))
				del(I)
				var/turf/T = get_turf_or_move(src.loc)
				for (var/mob/M in viewers(T))
					M.show_message("\red [pai.cable] rapidly retracts back into its spool.", 3, "\red You hear a click and the sound of wire spooling rapidly.", 2)

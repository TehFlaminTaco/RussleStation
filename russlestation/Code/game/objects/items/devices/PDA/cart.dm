	...
	if(mode==49)
		...
		for(var/obj/structure/mopbucket/B in world)
			...
			if(bl)
				...
				BucketData[++BucketData.len] = list ("x" = bl.x, "y" = bl.y, "dir" = uppertext(dir2text(direction)), "status" = B.reagents.total_volume)
		...
		for(var/obj/structure/janitorialcart/B in world)
			...
			if(bl)
				if(bl.z != cl.z)
					continue
				var/direction = get_dir(src,B)
				CartData[++CartData.len] = list("x" = bl.x, "y" = bl.y, "dir" = uppertext(dir2text(direction)), "status" = B.reagents.total_volume)
		if(!CartData.len)
		...
		for(var/obj/structure/stool/bed/chair/cart/janicart/B in world)
			...
			if(bl)
				if(bl.z != cl.z)
					continue
				var/direction = get_dir(src,B)
				PimpData[++PimpData.len] = list("x" = bl.x, "y" = bl.y, "dir" = uppertext(dir2text(direction)), "status" = B.reagents.total_volume)
		if(!PimpData.len)
			PimpData[++PimpData.len] = list("x" = 0, "y" = 0, dir=null, status = null)
		var/TankData[0]
		for(var/obj/item/weapon/reagent_containers/glass/chemtank/janitank/B in world)
			var/turf/bl = get_turf(B)
			if(bl)
				if(bl.z != cl.z)
					continue
				var/direction = get_dir(src,B)
				TankData[++TankData.len] = list("x" = bl.x, "y" = bl.y, "dir" = uppertext(dir2text(direction)), "status" = B.reagents.total_volume)
		if(!TankData.len)
			TankData[++TankData.len] = list("x" = 0, "y" = 0, dir=null, status = null)
		...
		JaniData["Pimp"] = PimpData
		JaniData["Tank"] = TankData

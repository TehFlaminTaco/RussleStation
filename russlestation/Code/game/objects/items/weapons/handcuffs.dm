...
/mob/living/carbon/human/RestrainedClickOn(var/atom/A)
	if (A != src)
		...
		var/headdam = ran_zone("head")
		var/armor = ML.run_armor_check(dam_zone, "melee")
		var/srcarmor = src.run_armor_check(headdam, "melee")
		if (prob(60)) //Arbitrary number is arbitrary.
			ML.apply_damage(rand(3, 8), BRUTE, dam_zone, armor)
			src.apply_damage(rand(10, 20), BRUTE, headdam, srcarmor)
			for (var/mob/O in viewers(ML, null))
				if (O!=src)
					O.show_message("\red <B>[name] has headbutted [ML]!</B>", 1)
				else
					src << "\red <B>You headbutt [ML]!</B>"
		else
			for(var/mob/O in viewers(ML, null))
				if (O!=src)
					O.show_message("\red <B>[name] has attempted to headbutt [ML]!</B>", 1)
				else
					src << "\red <B>You try to headbutt [ML]!</B>"
	else

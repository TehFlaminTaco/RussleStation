...
//				detonator.loc=src.loc
		...
		if(stage > 1 && !active && clown_check(user))
			...
			var/log_str = "[src] activated: <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[bombturf.x];Y=[bombturf.y];Z=[bombturf.z]'>[A.name]</a> "
		...
		if(src.reagents.total_volume) //The possible reactions didnt use up all reagents.
			...
			for(var/atom/A in view(affected_area, src.loc))
				...
				if(istype(A,/mob/living/carbon))
					...
					if(!C.wear_mask)
						src.reagents.trans_to_ingest(C, reagents.total_volume-5)
						src.reagents.reaction(C, 1, 5)
					else

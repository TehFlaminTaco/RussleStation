...
/obj/item/weapon/screwdriver/attackby(obj/item/W as obj, mob/user as mob)
	if(!istype(src.loc,/mob/living/carbon))
		return
	if(istype(W,/obj/item/weapon/card/emag_broken))
		return
	if(istype(W,/obj/item/weapon/card/emag/screwdriver))
		return
	if(istype(W,/obj/item/weapon/card/emag))
		var/obj/item/weapon/card/emag/E = W
		if(iscarbon(user))
			var/mob/living/carbon/C = user
			W.loc = get_turf(C.loc)
			var/olduses = E.uses
			del(W)
			var/obj/item/weapon/card/emag/screwdriver/F = new/obj/item/weapon/card/emag/screwdriver(C.loc)
			if (C.client)
				C.client.screen -= src
			if (C.r_hand == src)
				C.u_equip(src)
...
/obj/item/weapon/weldingtool/afterattack(atom/O as obj|mob, mob/user as mob, proximity)
	if(!proximity) return
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && get_dist(src,O) <= 1 && !src.welding)
		...
		return
	if (istype(O, /obj/item/weapon/reagent_containers/glass/chemtank) && get_dist(src,O) <= 1 && !src.welding)
		var/obj/item/weapon/reagent_containers/glass/chemtank/D = O
		if(!D.fillup)
			for(var/datum/reagent/R in D.reagents.reagent_list)
				if(R.id == "fuel")
					if(src.reagents.total_volume != src.reagents.maximum_volume)
						D.reagents.trans_to(src, max_fuel)
						user << "\blue Welder refueled"
						playsound(src.loc, 'sound/effects/refill.ogg', 50, 1, -6)
						return
	if (src.welding)

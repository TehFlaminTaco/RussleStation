...
/obj/machinery/implantchair
	...
	attackby(var/obj/item/G as obj, var/mob/user as mob)
		...
		if(istype(G, /obj/item/borg/grab))
			if(!ismob(G:attack))
				return
			for(var/mob/living/carbon/slime/M in range(1,G:attack))
				if(M.Victim == G:attack)
					usr << "[G:attack:name] will not fit into the [src.name] because they have a slime latched onto their head."
					return
			var/mob/M = G:attack
			if(put_mob(M))
				G:attack = null

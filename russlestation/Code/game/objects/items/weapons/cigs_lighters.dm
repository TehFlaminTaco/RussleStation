...
/obj/item/clothing/mask/cigarette/process()
	...
	if(reagents && reagents.total_volume)	//	check if it has any reagents at all
		if(iscarbon(loc) && (src == loc:wear_mask)) // if it's in the human/monkey mouth, transfer reagents to the mob
			...
			if(ishuman(loc) && (loc:species.flags & IS_SYNTHETIC)) return
	...
	if(lit)
		...
		if(hasorgans(M))
			...
			if(!(S.status & ORGAN_ROBOT) || user.a_intent != "help")
				M.adjustFireLoss(1)
				if(user != M)
					user.visible_message("\red \The [user] burns [M] with \the [src]",\
					"\red You burn [M] with \the [src]]",\
					"You hear a burning sound.")
					return
				else
					user.visible_message("\red \The [user] burns themself with \the [src]",\
					"\red You burn yourself with \the [src]",\
					"You hear a burning sound.")
					return
			if(S.brute_dam)

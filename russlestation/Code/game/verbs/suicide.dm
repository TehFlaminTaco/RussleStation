...
/mob/living/carbon/human/verb/suicide()
	...
	if(!src.client.holder)
		if(!src.client.has_antag(src))
			if(istype(usr.loc.loc,/area/security/))
				usr << "\red You cannot suicide while in security and not dead"
				return
			if(istype(usr.loc.loc,/area/prison/))
				usr << "\red You cannot suicide while in prison and not dead"
				return
			if(istype(usr.loc.loc,/area/shuttle/prison/))
				usr << "\red You cannot suicide while in prison and not dead"
				return
		...
		if(held_item)
			...
			if(damagetype)
				...
				if(!(damagetype | BRUTELOSS) && !(damagetype | FIRELOSS) && !(damagetype | TOXLOSS) && !(damagetype | OXYLOSS))
					if(species && (species.flags & NO_BREATHE))
						adjustFireLoss(max(175 - getToxLoss() - getFireLoss() - getBruteLoss() - getOxyLoss(), 0))
					else
...
/mob/living/carbon/monkey/verb/suicide()
	...
	if(confirm == "Yes")
		...
		if(istype(src,/mob/living/carbon/monkey/diona))
			adjustFireLoss(max(300- getToxLoss() - getFireLoss() - getBruteLoss() - getOxyLoss(), 0))
			sleep(10)
			death(0)
		else
			adjustOxyLoss(max(175- getToxLoss() - getFireLoss() - getBruteLoss() - getOxyLoss(), 0))
...
/mob/living/silicon/pai/verb/suicide()
	...
	if(istype(src.current, /obj/item/clothing/head/helmet))
		usr << "\red Cannot be used while in holographic form"
		return
	var/answer = input("REALLY kill yourself? This action can't be undone.", "Suicide", "No") in list ("Yes", "No")
	if(answer == "Yes")
		if(!istype(loc,/obj/item/device/paicard))
			usr << "\red you can't do this at this time."
			return

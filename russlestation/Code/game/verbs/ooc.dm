...
/client/verb/ooc(msg as text)
	...
	if(vipholder)
		if(!vipooc_allowed)
			src << "\red VIP OOC is globally muted"
			return
	...
	if((holder && !holder.fakekey) || vipholder)
		...
		if(holder && holder.rights & R_MOD && !(holder.rights & R_ADMIN))
			display_colour = "#184880"	//dark blue
		if(holder && holder.rights & R_DEBUG && !(holder.rights & R_ADMIN))
			display_colour = "#1b521f"	//dark green
		if(!(holder) && vipholder && vipholder.rights & V_VIPDEFAULT && !(vipholder.rights & V_SUPER))
			display_colour = "#bf4e87" //PINK
		if(!(holder) && vipholder && vipholder.rights & V_SUPER)
			display_colour = "#ff0066" //Hot Pink
		else if(holder && holder.rights & R_ADMIN)

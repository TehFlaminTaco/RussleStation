...
/turf/simulated/New()
	..()
	levelupdate()
/turf/HasProximity(atom/movable/AM as mob|obj)
	return
			...
			if(1)
				if(istype(M, /mob/living/carbon/human)) // Added check since monkeys don't have shoes
					if ((M.m_intent == "run") && !(istype(M:shoes, /obj/item/clothing/shoes) && M:shoes.flags&NOSLIP))
						...
						step(M, M.dir)
						var/blood = 0
						for(var/B in src.contents)
							if(istype(B, /obj/effect/decal/cleanable/blood))
								for(var/obj/effect/decal/cleanable/blood/BP in src.contents)
									if(BP.amount >= 1)
										blood = 1

						if(blood)
							M << "\blue You slipped on the wet blood!"
						else

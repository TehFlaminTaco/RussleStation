...
/obj/machinery/SP_fabricator
	...
	"Civilian Space Pod parts"=list(/obj/item/SP_parts/chassis/civ,
	...
	"Gold Civilian Space Pod parts"=list(/obj/item/SP_parts/chassis/gciv,
						/obj/item/SP_parts/part/gciv_plat,
						/obj/item/SP_parts/part/gciv_armor,
						/obj/item/SP_parts/part/window,
						/obj/item/SP_parts/part/engine
					),
	"Military Space Pod parts"=list(/obj/item/SP_parts/chassis/mil,
						/obj/item/SP_parts/part/mil_plat,
						/obj/item/SP_parts/part/mil_armor,
						/obj/item/SP_parts/part/window,
						/obj/item/SP_parts/part/engine
					),
	"Industrail Space Pod parts"=list(/obj/item/SP_parts/chassis/indy,
						/obj/item/SP_parts/part/indy_plat,
						/obj/item/SP_parts/part/indy_armor,
						/obj/item/SP_parts/part/window,
						/obj/item/SP_parts/part/engine
					),
	"Honk Pod parts"=list(/obj/item/SP_parts/chassis/honk,
...
/obj/machinery/SP_fabricator/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/weapon/circuitboard/SPfab(src)
...
/obj/machinery/SP_fabricator/RefreshParts()
	var/T = 0
...
/obj/machinery/SP_fabricator/Del()
	for(var/atom/A in src)
		del A
	..()
	return

/obj/machinery/SP_fabricator/proc/operation_allowed(mob/M)
	if(isrobot(M) || isAI(M))
		return 1
...
/obj/machinery/SP_fabricator/check_access(obj/item/weapon/card/id/I)
...
/obj/machinery/SP_fabricator/proc/emag()
	sleep()
	switch(emagged)
		if(0)
			emagged = 0.5
...
/obj/machinery/SP_fabricator/proc/convert_part_set(set_name as text)
...
/obj/machinery/SP_fabricator/proc/add_part_set(set_name as text,parts=null)
...
/obj/machinery/SP_fabricator/proc/add_part_to_set(set_name as text,part)
	if(!part) return 0
...
/obj/machinery/SP_fabricator/proc/remove_part_set(set_name as text)
...
/obj/machinery/SP_fabricator/proc/output_parts_list(set_name)
	var/output = ""
...
/obj/machinery/SP_fabricator/proc/output_part_info(var/obj/item/part)
	var/output = "[part.name] (Cost: [output_part_cost(part)]) [get_construction_time_w_coeff(part)/10]sec"
	return output

/obj/machinery/SP_fabricator/proc/output_part_cost(var/obj/item/part)
	var/i = 0
	var/output
...
/obj/machinery/SP_fabricator/proc/output_available_resources()
	var/output
...
/obj/machinery/SP_fabricator/proc/remove_resources(var/obj/item/part)
...
/obj/machinery/SP_fabricator/proc/check_resources(var/obj/item/part)
...
/obj/machinery/SP_fabricator/proc/build_part(var/obj/item/part)
	if(!part) return
	...
	if(being_built)
		if(istype(being_built,/obj/item/device/spacepod_equipment/weaponry))
			var/obj/item/device/spacepod_equipment/weaponry/W = being_built
			if(W.locked)
				var/obj/item/weapon/storage/lockbox/L = new/obj/item/weapon/storage/lockbox(get_step(src,SOUTH))
				being_built.loc = L
...
/obj/machinery/SP_fabricator/proc/update_queue_on_page()
	send_byjax(usr,"SP_fabricator.browser","queue",src.list_queue())
	return

/obj/machinery/SP_fabricator/proc/add_part_set_to_queue(set_name)
...
/obj/machinery/SP_fabricator/proc/add_to_queue(part)
	if(!istype(queue))
		queue = list()
	if(part)
...
/obj/machinery/SP_fabricator/proc/remove_from_queue(index)
	if(!isnum(index) || !istype(queue) || (index<1 || index>queue.len))
		return 0
	queue.Cut(index,++index)
	return 1

/obj/machinery/SP_fabricator/proc/process_queue()
...
/obj/machinery/SP_fabricator/proc/list_queue()
...
/obj/machinery/SP_fabricator/proc/convert_designs()
	if(!files) return
	var/i = 0
...
/obj/machinery/SP_fabricator/proc/update_tech()
	if(!files) return
	var/output
...
/obj/machinery/SP_fabricator/proc/sync(silent=null)
/*		if(queue.len)
			if(!silent)
...
/obj/machinery/SP_fabricator/proc/get_resource_cost_w_coeff(var/obj/item/part as obj,var/resource as text, var/roundto=1)
//Be SURE to add any new equipment to this switch, but don't be suprised if it spits out children objects
	if(part.vars.Find("construction_time") && part.vars.Find("construction_cost"))
...
/obj/machinery/SP_fabricator/proc/get_construction_time_w_coeff(var/obj/item/part as obj, var/roundto=1)
//Be SURE to add any new equipment to this switch, but don't be suprised if it spits out children objects
	if(part.vars.Find("construction_time") && part.vars.Find("construction_cost"))
...
/obj/machinery/SP_fabricator/attack_hand(mob/user as mob)
	var/dat, left_part
	if (..())
		return
	...
	user << browse(dat, "window=SP_fabricator;size=1000x400")
	onclose(user, "SP_fabricator")
	return

/obj/machinery/SP_fabricator/proc/exploit_prevention(var/obj/Part, mob/user as mob, var/desc_exploit)
...
/obj/machinery/SP_fabricator/Topic(href, href_list)
...
/obj/machinery/SP_fabricator/proc/remove_material(var/mat_string, var/amount)
	var/type
	switch(mat_string)
		if("metal")
...
/obj/machinery/SP_fabricator/attackby(obj/W as obj, mob/user as mob)

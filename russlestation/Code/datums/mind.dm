...
datum/mind
	...
	proc/edit_memory()
		...
		out += "Factions and special roles:[special_role]<br>"
...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
	...
	Topic(href, href_list)
		...
		if (href_list["role_edit"])
			...
			var/new_obj_type = input("Select objective type:", "Objective type", def_value) as null|anything in list("assassinate","blood", "debrain", "protect", "prevent", "harm", "brig", "hijack", "escape", "survive", "steal", "steal money", "download", "nuclear", "capture", "absorb", "custom")
			...
			switch (new_obj_type)
				...
				if ("steal")
					...
					if (!steal.select_target())
						return
				if ("steal money")
					if(!istype(objective, /datum/objective/stealmoney))
						new_objective = new /datum/objective/stealmoney
						new_objective.target_amount = input("input target amount:", "Objective", new_objective.target_amount) as num|null
						if(isnull(new_objective.target_amount))
							new_objective.target_amount = rand(10000,35000)
						new_objective.explanation_text = "Steal $[new_objective.target_amount] from the station by the end of the shift.(Must be on you or in your account.)"
			...
			switch(href_list["changeling"])
				if("clear")
					if(src in ticker.mode.changelings)
						...
						current.verbs -= /datum/changeling/proc/EvolutionMenu
						if(current.hud_used)
							if(current.hud_used.changeling_chem_display)
								current.client.screen -= current.hud_used.changeling_chem_display
								current.hud_used.changeling_chem_display = null
		...
		else if (href_list["assimilate"])
			current.hud_updateflag |= (1 << SPECIALROLE_HUD)
			switch(href_list["assimilate"])
				if("clear")
					if(src in ticker.mode.Syndicateborg)
						if(src in ticker.mode.orginalSyndicateborg)
							ticker.mode.orginalSyndicateborg -=src
						ticker.mode.Syndicateborg -= src
						current.faction = "neutral"
						special_role = null
						current << "<FONT color='red' size = 3><B>You Reboot! Deleting Assimilation Subroutines.....You no longer work for the syndicate!</B></FONT>"
						for(var/obj/item/clothing/gloves/assim/D in current.contents)
							del(D)
						if(istype(current, /mob/living/carbon/human))
							var/mob/living/carbon/human/E = current
							for(var/datum/organ/external/head/F in E.organs)
								F.icon_name = "head"
							E.real_name = name
							E.name = name
							E.r_fur = 1
							...
							for(var/datum/objective/objective in src.objectives)
								if(!istype(objective))	return
								src.objectives -= objective
						src.current.client.images = list()
						current.regenerate_icons()
						ticker.mode.update_all_Syndicateborg_icons()
						log_admin("[key_name_admin(usr)] has de-Assimilated [current].")
				if("assimilate")
					if(ticker && !istype(ticker.mode,/datum/game_mode/assimilate))return
					if(!(src in ticker.mode.Syndicateborg))
						ticker.mode.Syndicateborg += src
						special_role = "Syndicate Assimilation Borg"//So they actually have a special role/N
						if(istype(src.current, /mob/living/carbon/human))
							var/mob/living/carbon/human/D = src.current
							if(D.species.name != "Machine")
								D.set_species("Machine")
						assigned_role = "MODE" //So they aren't chosen for other jobs.
						ticker.mode.forge_Syndicateborg_objectives(src)
						ticker.mode.greet_Syndicateborg(src)
						ticker.mode.equip_new_Syndicateborg(src.current,1)
						src.current.revive()
						log_admin("[key_name_admin(usr)] has Assimilated [current].")
				if("autoobjectives")
					if(!config.objectives_disabled)
						return
					usr << "\blue The objectives for vampire [key] have been generated. You can edit them and announce manually."

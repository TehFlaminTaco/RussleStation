...
//	/datum/admins/proc/show_traitor_panel,	/*interface which shows a mob's mind*/ -Removed due to rare practical use. Moved to debug verbs ~Errorage
	...
	/datum/admins/proc/view_access_log,
...
//	/client/proc/toggle_hear_deadcast,	/*toggles whether we hear deadchat*/
	...
	/client/proc/cmd_admin_change_custom_event, /*Announces a set event message to people joining the server*/
    ...
    /client/proc/hideasay,
    /client/proc/hidemsay,
    ...
    /datum/admins/proc/togglevipooc,    /*toggles vip ooc on/off for VIPs*/
...
var/list/admin_verbs_permissions = list(
	...
	/datum/admins/proc/togglemsay,    /*toggles msay on/off for everyone*/
...
/client/proc/editappear(mob/living/carbon/human/M as mob in world)
	...
	if(M.species.name == "Human")
		var/new_tone = input("Please select skin tone level: 1-220 (1=albino, 35=caucasian, 150=black, 220='very' black)", "Character Generation")  as text

		if (new_tone)
			M.s_tone = max(min(round(text2num(new_tone)), 220), 1)
			M.s_tone =  -M.s_tone + 35
	if(M.species.flags & HAS_COLOR)
		var/new_fur = input("Please select fur color.", "Character Generation") as color
		if(new_fur)
			M.r_fur = hex2num(copytext(new_fur, 2, 4))
			M.g_fur = hex2num(copytext(new_fur, 4, 6))
			M.b_fur = hex2num(copytext(new_fur, 6, 8))
...
/client/proc/toggleattacklogs()
	...
	set category = "Prefs A/M"

	prefs.Atoggles ^= CHAT_ATTACKLOGS
	if (prefs.Atoggles & CHAT_ATTACKLOGS)
		usr << "You now will get attack log messages"
	else
		usr << "You now won't get attack log messages"

/client/proc/hideasay()
	set name = "Hide/Show Admin Chat"
	set category = "Prefs A/M"

	prefs.Atoggles ^= CHAT_ASAY
	prefs.save_preferences()
	if (prefs.Atoggles & CHAT_ASAY)
		usr << "You now will See Admin Chat"
	else
		usr << "You now won't See Admin Chat"

/client/proc/hidemsay()
	set name = "Hide/Show Mod Chat"
	set category = "Prefs A/M"

	prefs.Atoggles ^= CHAT_MSAY
	prefs.save_preferences()
	if (prefs.Atoggles & CHAT_MSAY)
		usr << "You now will See Mod Chat"
	else
		usr << "You now won't See Mod Chat"

/client/proc/hidedevsay()
	set name = "Hide/Show Dev Chat"
	set category = "Prefs A/M"

	prefs.Atoggles ^= CHAT_DEVSAY
	prefs.save_preferences()
...
/client/proc/hidevsay()
	set name = "Hide/Show VIP Chat"
	set category = "Prefs A/M"

	prefs.Atoggles ^= CHAT_VSAY
	prefs.save_preferences()
	if (prefs.Atoggles & CHAT_VSAY)
		usr << "You now will See VIP Chat"
	else
		usr << "You now won't See VIP Chat"
...
/client/proc/toggledebuglogs()
	...
	prefs.Atoggles ^= CHAT_DEBUGLOGS
	if (prefs.Atoggles & CHAT_DEBUGLOGS)
...
/client/proc/toggle_attacks()
	set category ="Special Verbs"
	set name = "Toggle Attacks(WIP)"
	set desc = "toggles all attacks on or off"
	if(!holder)
		return
	if(!check_rights(R_SERVER)) return
	if(no_attack == 0)
		no_attack = 1
		world << "\red <b>ADMIN Toggled Attacks off</b>"
	else
		no_attack = 0
		world << "\red <b>ADMIN Toggled Attacks on</b>"

	log_admin("[key_name(usr)] toggled attacks")
	message_admins("\blue [key_name_admin(usr)] toggled attacks", 1)

/datum/admins/Topic(href, href_list)
	...
	if(href_list["makeAntag"])
	...
	else if(href_list["call_crew_transfer"])
		...
		if(!emergency_shuttle.online)
			...
			if(confirm != "Yes")
				...
				feedback_add_details("admin_verb","CTRANS2") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
				log_admin("[key_name(usr)] admin-called a shift change.")
				message_admins("\blue [key_name_admin(usr)] admin-called a shift change.", 1)
		else
			usr << "\red Shuttle Has already been called."
	...
	/////////////////////////////////////new ban stuff
		...
		message_mods("\blue [key_name_admin(usr)] edited [banned_key]'s ban. Reason: [reason] Duration: [duration]", 1)
		Banlist.cd = "/base/[banfolder]"
...
*/
		...
		if(jobban_isbanned(M, "Diona"))
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Diona;jobban4=\ref[M]'><font color=red>Diona</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Diona;jobban4=\ref[M]'>Diona</a></td>"

		//vip vox
		if(jobban_isbanned(M, "VIP VOX"))
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=VIP VOX;jobban4=\ref[M]'><font color=red>VIP VOX</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=VIP VOX;jobban4=\ref[M]'>VIP VOX</a></td>"

		//unathi
		if(jobban_isbanned(M, "Unathi"))
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Unathi;jobban4=\ref[M]'><font color=red>Unathi</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Unathi;jobban4=\ref[M]'>Unathi</a></td>"

		//Aviskree
		if(jobban_isbanned(M, "Aviskree"))
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Aviskree;jobban4=\ref[M]'><font color=red>Aviskree</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Aviskree;jobban4=\ref[M]'>Aviskree</a></td>"

		//Tajaran
		if(jobban_isbanned(M, "Tajaran"))
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Tajaran;jobban4=\ref[M]'><font color=red>Tajaran</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Tajaran;jobban4=\ref[M]'>Tajaran</a></td>"

		//Skrell
		if(jobban_isbanned(M, "Skrell"))
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Skrell;jobban4=\ref[M]'><font color=red>Skrell</font></a></td>"
		else
			jobs += "<td width='20%'><a href='?src=\ref[src];jobban3=Skrell;jobban4=\ref[M]'>Skrell</a></td>"
	...
	//JOBBAN'S INNARDS
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					message_mods("\blue [key_name_admin(usr)] banned [key_name_admin(M)] from [msg] for [mins] minutes", 1)
				...
				if("No")
					...
					if(reason)
						...
						message_mods("\blue [key_name_admin(usr)] banned [key_name_admin(M)] from [msg]", 1)
						M << "\red<BIG><B>You have been jobbanned by [usr.client.ckey] from: [msg].</B></BIG>"
						M << "\red <B>The reason is: [reason]</B>"
		...
		if(joblist.len) //at least 1 banned job exists in joblist so we have stuff to unban.
			...
			if(msg)
				...
				message_mods("\blue [key_name_admin(usr)] unbanned [key_name_admin(M)] from [msg]", 1)
		...
		if (ismob(M))
			...
			if(!reason)
				...
				notes_add(M.ckey, "Booted by [key_name(usr)] - no reason")
			else
				M << "\red You have been kicked from the server: [reason]"
				notes_add(M.ckey, "Booted  for - [reason] by [key_name(usr)]")
			log_admin("[key_name(usr)] booted [key_name(M)].")
			message_admins("\blue [key_name_admin(usr)] booted [key_name_admin(M)].", 1)
			message_mods("\blue [key_name_admin(usr)] booted [key_name_admin(M)].", 1)
			//M.client = null
			del(M.client)
/*
	//Player Notes
...
*/
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				notes_add(M.ckey, "[usr.client.ckey] has Temp Banned [M.ckey] for [reason] ban time =[mins]")
				...
				message_mods("\blue[usr.client.ckey] has banned [M.ckey].\nReason: [reason]\nThis will be removed in [mins] minutes.")
			...
			if("No")
				...
				notes_add(M.ckey, "[usr.client.ckey] has Perma Banned [M.ckey] for [reason] ban time = Permanent")
				M << "\red<BIG><B>You have been banned by [usr.client.ckey].\nReason: [reason].</B></BIG>"
	...
	else if(href_list["makevox"])
		if(!check_rights(R_SPAWN))	return

		var/mob/living/carbon/human/H = locate(href_list["makevox"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/human"
			return
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		if (number == 1)
			...
			for(var/path in paths)
				if(ispath(path, /obj))
					message_admins("[key_name_admin(usr)] spawned a [english_list(paths)]", 1)
					break
		else
			...
			for(var/path in paths)
				if(ispath(path, /mob))
					...
					break
				if(ispath(path, /obj))
					message_admins("[key_name_admin(usr)] spawned [number] of [english_list(paths)]", 1)
					break
		return
...
//don't warp them if they aren't ready or are already there
			...
			if("moveertshuttle")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","ShPie")
				if(specops_shuttle_at_station == 0)
					specops_process()
				else if(specops_shuttle_at_station == 1)
					specops_return()
				else
					usr << "\red Shuttle is an transit please wait and try again!"
				message_admins("\blue [key_name_admin(usr)] Moved the ERT Shuttle", 1)
				log_admin("[key_name(usr)] Moved the ERT Shuttle")
			if("moveferry")
				feedback_inc("admin_secrets_fun_used",1)
...
/*				if("shockwave")
	...
	if(href_list["vipeditrights"])
		if(!check_rights(R_PERMISSIONS))
			message_admins("[key_name_admin(usr)] attempted to edit the VIP permissions without sufficient rights.")
			log_admin("[key_name(usr)] attempted to edit the VIP permissions without sufficient rights.")
			return

		var/vip_ckey

		var/task = href_list["vipeditrights"]
		if(task == "add")
			var/new_ckey = ckey(input(usr,"New vip's ckey","Vip ckey", null) as text|null)
			if(!new_ckey)	return
			if(new_ckey in vip_datums)
				usr << "<font color='red'>Error: Topic 'editrights': [new_ckey] is already a vip</font>"
				return
			vip_ckey = new_ckey
			task = "rank"
		else if(task != "show")
			vip_ckey = ckey(href_list["ckey"])
			if(!vip_ckey)
				usr << "<font color='red'>Error: Topic 'editrights': No valid ckey</font>"
				return
		...
		if(task == "remove")
			if(alert("Are you sure you want to remove [vip_ckey]?","Message","Yes","Cancel") == "Yes")
				if(!D)	return
				vip_datums -= vip_ckey
				D.disassociate()

				message_admins("[key_name_admin(usr)] removed [vip_ckey] from the vips list")
				log_admin("[key_name(usr)] removed [vip_ckey] from the vips list")
				log_vip_rank_modification(vip_ckey, "Removed")

		else if(task == "rank")
			var/new_rank
			if(vip_ranks.len)
				new_rank = input("Please select a rank", "New rank", null, null) as null|anything in (vip_ranks|"*New Rank*")
			else
				new_rank = input("Please select a rank", "New rank", null, null) as null|anything in list("VIP","SuperVIP", "*New Rank*")
			...
			switch(new_rank)
				...
				if("*New Rank*")
					...
					if(config.vip_legacy_system)
						if(vip_ranks.len)
							if(new_rank in vip_ranks)
								rights = vip_ranks[new_rank]		//we typed a rank which already exists, use its rights
							else
								vip_ranks[new_rank] = 0			//add the new rank to vip_ranks
				else
					if(config.vip_legacy_system)
						new_rank = ckeyEx(new_rank)
						rights = vip_ranks[new_rank]				//we input an existing rank, use its rights

			if(D)
				D.disassociate()								//remove vipverbs and unlink from client
				D.rank = new_rank								//update the rank
				D.rights = rights								//update the rights based on vip_ranks (default: 0)
			else
				D = new /datum/vips(new_rank, rights, vip_ckey)
			...
			if(!D)	return
			var/list/permissionlist = list()
			for(var/i=1, i<=V_VIPMAXPERMISSION, i<<=1)		//that <<= is shorthand for i = i << 1. Which is a left bitshift
				permissionlist[vip_rights2text(i)] = i
			var/new_permission = input("Select a permission to turn on/off", "Permission toggle", null, null) as null|anything in permissionlist
			if(!new_permission)	return
			D.rights ^= permissionlist[new_permission]

			message_admins("[key_name_admin(usr)] toggled the [new_permission] permission of [vip_ckey]")
			log_admin("[key_name(usr)] toggled the [new_permission] permission of [vip_ckey]")
			log_vip_permission_modification(vip_ckey, permissionlist[new_permission])

		edit_vip_permissions()

...
/datum/admins/proc/view_access_log()
	set category = "Admin"
	set name = "Show Access Log"
	set desc = "Shows today's Access log."

	var/path = "data/logs/[time2text(world.realtime,"YYYY/MM-Month/DD-Day")] Access.log"
	if( fexists(path) )
		src << run( file(path) )
	else
		src << "<font color='red'>Error: view_txt_log(): File not found/Invalid path([path]).</font>"
		return
	feedback_add_details("admin_verb","VAcL") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	return
...
/datum/admins/proc/view_admin_log()
	...
	var/path = "data/logs/[time2text(world.realtime,"YYYY/MM-Month/DD-Day")] Adminlog.log"
	if( fexists(path) )
		src << run( file(path) )
	else
		src << "<font color='red'>Error: view_txt_log(): File not found/Invalid path([path]).</font>"
		return
	feedback_add_details("admin_verb","VAlL") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	return
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if (H.wear_id)
			...
			if(istype(H.wear_id, /obj/item/device/pda))
				...
				var/obj/item/weapon/storage/wallet/wallet = H.wear_id
				id = wallet.front_id
				id.icon_state = "gold"
				wallet.update_icon()
			id.icon_state = "gold"
...
/client/proc/cmd_assume_direct_control(var/mob/M in mob_list)
	...
	if(ishuman(M))
		...
		var/oldspecies = D.species
		D.ckey = src.ckey
		D.species = oldspecies
...
/client/proc/cmd_admin_dress(var/mob/living/carbon/human/M in mob_list)
	...
	switch(dresscode)
		...
		if("masked killer")
			...
			M.equip_to_slot_or_del(new /obj/item/clothing/mask/surgical(M), slot_neck)
		...
		if("engineer w/ hardsuit")
			...
			if(!M.wear_id)
				...
				W.name = "[M.real_name]'s ID Card"
				W.icon_state = "id_eng"
				W.access = list(access_eva, access_engine, access_engine_equip, access_tech_storage, access_maint_tunnels, access_external_airlocks, access_construction, access_atmospherics)
				W.assignment = "Station Engineer"
				W.registered_name = M.real_name
				M.equip_to_slot_or_del(W, slot_wear_id)
		if("emmanuel's tactical")
			...
			M.equip_to_slot_or_del(new /obj/item/clothing/head/helmet/tactical(M), slot_head)
			M.equip_to_slot_or_del(new /obj/item/clothing/mask/balaclava/tactical(M), slot_wear_mask)
			M.equip_to_slot_or_del(new /obj/item/clothing/suit/armor/tactical(M), slot_wear_suit)
			M.equip_to_slot_or_del(new /obj/item/device/radio/headset/ert(M), slot_l_ear)
			M.equip_to_slot_or_del(new /obj/item/weapon/gun/energy/pulse_rifle/M1911(M), slot_s_store)
			M.equip_to_slot_or_del(new /obj/item/device/pda/security(M), slot_wear_pda)
			M.equip_to_slot_or_del(new /obj/item/clothing/gloves/black(M), slot_gloves)
			...
			if(!M.wear_id)
				...
				W.access = get_all_accesses()
				W.access += get_all_centcom_access()
				W.assignment = "Intergalactic Police"
				W.registered_name = M.real_name
				M.equip_to_slot_or_del(W, slot_wear_id)
			var/obj/item/weapon/storage/belt/security/tactical/B = new(M)
			B.contents += new /obj/item/weapon/melee/classic_baton(B)
			B.contents += new /obj/item/weapon/handcuffs(B)
			B.contents += new /obj/item/weapon/handcuffs(B)
			M.equip_to_slot_or_del(B, slot_belt)
...
/client/proc/cmd_debug_mob_lists()
	...
	switch(input("Which list?") in list("Players","Admins", "Vips","Mobs","Living Mobs","Dead Mobs", "Clients", "Processing_objs","research","antags"))
		if("Players")
		...
		if("Processing_objs")
			usr << list2text(processing_objects, ",<BR>")
		if("research")
			for(var/datum/design/D in research_list)
				var/entry = ""
				entry += "<B>Name:</B> [D.name] <BR>"
				entry +="<B>Reseach: </B> "
				entry += "[list2params(D.req_tech)]"
				entry +=  "<BR>"
				entry += "<B>Materials:</B> "
				entry += "[list2params(D.materials)]"
				entry +=  "<BR>"
				entry += "<B>Locked:</B> [D.locked == 1 ? "Yes": "No"]<BR><BR>"
				usr << entry

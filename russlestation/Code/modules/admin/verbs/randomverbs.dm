...
/client/proc/allow_character_respawn()
	...
	G.timeofdeath=-19999						/* time of death is checked in /mob/verb/abandon_mob() which is the Respawn verb.
									...
									*/
	if (G.client)
		G.client.time_died_as_mouse=-19999 //That should help fix a few things.
...
/client/proc/toggle_antagHUD_use()
	...
	if(config.antag_hud_allowed)
		for(var/mob/dead/observer/g in get_ghosts())
			if((g.client && !g.client.holder))	//Remove the verb from non-admin ghosts
		...
		for(var/mob/dead/observer/g in get_ghosts())
			if((g.client && !g.client.holder))// Add the verb back for all non-admin ghosts
				g.verbs += /mob/dead/observer/verb/toggle_antagHUD
	...
	*/
	...
	switch(new_character.mind.special_role)
		...
		if("Ninja")
			...
			if(ninjastart.len == 0)
				var/obj/effect/landmark/ninjastart = locate("landmark*ninjastart")
				if(ninjastart)
					ninjastart += get_turf(ninjastart)
			if(ninjastart.len == 0)
...
/client/proc/admin_vox()
	...
	if(ticker && istype(ticker.mode,/datum/game_mode/heist))
		if(alert("Are you sure you want to become a Vox? The Gamemode is Heist.",,"Yes","No")=="No")
			return
...
/client/proc/admin_vox_ship_tele()
...
/client/proc/ADMIN_Borg_Sprite()
	...
	if (!istype(M, /mob/living/silicon/robot))
		M << "\red You Must be a Cyborg for this to work"
		return

	var/mob/living/silicon/robot/D = usr
	if (istype(D, /mob/living/silicon/robot))
		var/previous_icon = D.icon_state
		if(D.modtype == "Default")
			var/icontype = ""
			icontype = input("Select an VIP Sprite!", "Cyborg", null, null) in list("Vip", "Sydrec", "Ravensdale", "Djsnapshot")
			switch(icontype)
				if("Vip")
		...
		if(D.modtype == "Surgery")
			var/icontype = ""
			icontype = input("Select an VIP Sprite!", "Cyborg", null, null) in list("Sydrec", "Ravensdale", "Djsnapshot")
			switch(icontype)
				if("Sydrec")
		...
		if(D.modtype == "Combat")
			var/icontype = ""
			icontype = input("Select an VIP Sprite!", "Cyborg", null, null) in list("Vip", "Sydrec", "Djsnapshot")
			switch(icontype)
				if("Vip")
...
client/proc/ADMIN_AI_Sprite()
	...
	if (istype(A, /mob/living/silicon/ai))
		...
		var/icontype = ""
		icontype = input("Select an VIP Sprite!", "ai", null, null) in list("Vip", "Serithi", "Ravensdale")
		switch(icontype)
...
/client/proc/adminchanges()
	...
	getFiles(
		...
		'html/changelog.css',
		'html/changelog.js',
		'html/adminchangelog.html'
		)
	src << browse('html/adminchangelog.html', "window=changes;size=675x650")
...
/client/proc/SpawnVIPstore()
	set name = "Spawn A VIP Store"
	set category = "Fun"
	if(!ticker)
		return
	if(usr.client.holder)
	...
	new/obj/item/VIP_store(usr.loc)
	verbs.Remove(/client/proc/SpawnVIPstore)


/client/proc/admin_call_crewtransfer()
	set name = "Call Crew Transfer"
	set category = "Admin"
...
/client/proc/adminpoints()
	set name = "+/- scoreboard admin points"
	set category = "Fun"
	if(!ticker)
		return
	if(usr.client.holder)
		if(!check_rights(R_ADMIN|R_MOD))
			return
	if(inuse==1)
		usr << "this verb is in use by another admin"
		return
	var/yon = alert(src, "These Points and titles can't be remove yet...do you wish to continue", "Confirm", "Yes", "No")
	if(yon != "Yes") return
	inuse = 1
	var/gorb = alert(src, "do you want to add or subtract points", "Which", "Add", "Subtract","Cancel")
	if(gorb == "Cancel")
		inuse = 0
		return
	if(gorb == "Add")
		adminpointsgood = 1
		var/name = input("Title:", text("Title for addition:")) as text
		var/add = input("Amount to be added","Amount") as num|null
		adminscoregoodtitle += name
		adminscoregoodtitle[name] += add
		inuse = 0
	if(gorb == "Subtract")
		adminpointsbad = 1
		var/name = input("Title:", text("Title for subtraction:")) as text
		var/subtract = input("Amount to be subtracted","Amount") as num|null
		adminscorebadtitle += name
		adminscorebadtitle[name] += subtract
		inuse = 0

/client/proc/lockershut()
	set name = "Shut All Lockers"
	set category = "Special Verbs"
	if(!ticker)
		return
	if(usr.client.holder)
	...
	for(var/obj/structure/closet/C in world)
		if(C.opened)
			C.close()
	message_admins("Admin [key_name_admin(usr)] has closed all Lockers in the world.", 1)
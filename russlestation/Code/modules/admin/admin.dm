...
/proc/msg_admin_attack(var/text) //Toggleable Attack Messages
	...
	for(var/client/C in admins)
		if(R_ADMIN & C.holder.rights)
			if(C.prefs.Atoggles & CHAT_ATTACKLOGS)
				var/msg = rendered
				C << msg
		...
		if(!istype(M, /mob/new_player))
			...
			if(isAI(M))
				...
				body += {"<A href='?src=\ref[src];makeai=\ref[M]'>Make AI</A> |
					...
					<A href='?src=\ref[src];makevox=\ref[M]'>Voxify</A> |
...
/datum/admins/proc/Secrets()
	...
	if(check_rights(R_FUN,0))
		dat += {"
			...
			<A href='?src=\ref[src];secretsfun=moveertshuttle'>Move ERT Shuttle</A><BR>
...
/datum/admins/proc/restart()
	...
	if(confirm == "Yes")
		if(check_rights(R_ADMIN,0))
			...
			world << "\red <b>Restarting world!</b> \blue Initiated by [usr.client.holder.fakekey ? "Mod" : usr.key]!"
...
/datum/admins/proc/announce()
	...
	//		message = adminscrub(message,500)
		if(check_rights(R_ADMIN,0))
			...
			world << "\blue <b>[usr.client.holder.fakekey ? "Moderator" : usr.key] Announces:</b>\n \t [message]"
...
/datum/admins/proc/togglemsay()
	set category = "Server"
	set desc="Globally Toggles MSAY"
	set name="Toggle MSAY"
	msay_allowed = !( msay_allowed )

	log_admin("[key_name(usr)] toggled (Mods)Mod chat.")
	message_admins("[key_name_admin(usr)] toggled (Mods)Mod chat.", 1)
	feedback_add_details("admin_verb","TMSAY") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc
...
/datum/admins/proc/togglevsay()
	...
	if (vsay_allowed)
		for(var/client/C in clients)
			if(C.vipholder)
				C << "<B>VIPchat has been globally enabled!</B>"

	else
		for(var/client/C in clients)
			if(C.vipholder)
				C << "<B>VIPchat has been globally disabled!</B>"
	log_admin("[key_name(usr)] toggled VIP chat.")
	message_admins("[key_name_admin(usr)] toggled VIP chat.", 1)
...
/datum/admins/proc/togglevipooc()
	set category = "Server"
	set desc="Globally Toggles vip ooc"
	set name="Toggle VIP OOC"
	vipooc_allowed = !( vipooc_allowed )
	if (vipooc_allowed)
		for(var/client/C in clients)
			if(C.vipholder)
				C << "<B>VIP OOC has been globally enabled!</B>"

	else
		for(var/client/C in clients)
			if(C.vipholder)
				C << "<B>VIP OOC has been globally disabled!</B>"
	log_admin("[key_name(usr)] toggled VIP OOC.")
	message_admins("[key_name_admin(usr)] toggled VIP OOC.", 1)
	feedback_add_details("admin_verb","TVOOC") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc
...
/datum/admins/proc/immreboot()
	...
	if(check_rights(R_ADMIN,0))
		...
		world << "\red <b>Rebooting world!</b> \blue Initiated by [usr.client.holder.fakekey ? "Mod" : usr.key]!"
...
/datum/admins/proc/spawn_atom(var/object as text)
	...
	if(!config.allow_admin_spawning) return

...
/obj/item/clothing/glasses/hud/thermal
	name = "Thermal HUD"
	desc = "A heads-up display that scans the humans in view and provides Thermal data."
	icon_state = "securityhud"
...
/obj/item/clothing/glasses/hud/thermal/process_hud(var/mob/M)
	...
	if(!M.client)	return
	var/client/C = M.client
	for(var/mob/living/carbon/human/perp in world)
		if(M.see_invisible < perp.invisibility)
			continue
		if(!C) continue
		C.images += perp.hud_list[THERMAL_HUD]

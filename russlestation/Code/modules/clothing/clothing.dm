/obj/item/clothing
	...
	var/baseslowdown = null //Initializing the new slowdown variable here until it's proven to be needed on other objects, too. ~Aztec
	var/digitigrade = null
...
/obj/item/clothing/mob_can_equip(M as mob, slot)
	...
	if(species_restricted && istype(M,/mob/living/carbon/human))
		...
		//Species based speed reduction. ~Aztec
	if(!(digitigrade) && slot_flags & SLOT_FEET) //If it's digitigrade compatible or not shoes, no need to mess with the speeds
		var/mob/living/carbon/human/H = M
		if (H.species.digitigrade) //If you're digitigrade, it'll slow you down a ton ~Aztec
			slowdown = baseslowdown + 3
			M << "\red [src] probably weren't made for you."
		else						//If not, it'll go to normal
...
/obj/item/clothing/suit/space
	name = "Space suit"
	desc = "A suit that protects against low pressure environments. \"NSS Horizon\" is written in large block letters on the back."
	icon_state = "space"
	item_state = "s_suit"

	...
	if(state >= GRAB_NECK)
		...
		if(isliving(affecting))
			...
			if(istype(L,/mob/living/carbon/human))
				var/mob/living/carbon/human/A = L
				if(A.species && (A.species.flags & NO_BREATHE))
					return
				else
					A.adjustOxyLoss(1)
			else
...
/obj/item/weapon/grab/proc/s_click(obj/screen/S)
	...
	if(state < GRAB_AGGRESSIVE)
		...
		if(state < GRAB_NECK)
			...
			log_attack("<font color='#386aff'>[assailant.name] ([assailant.ckey]) grabbed the neck of [affecting.name] ([affecting.ckey])</font>")
			...
			if(state < GRAB_UPGRADING)
				...
				if(do_after(assailant, UPGRADE_KILL_TIMER))
					...
					log_attack("[key_name(assailant)] strangled (kill intent) [key_name(affecting)]")

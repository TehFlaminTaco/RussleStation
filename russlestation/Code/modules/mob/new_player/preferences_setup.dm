datum/preferences
	...
	proc/update_preview_icon()		//seriously. This is horrendous.
		...
		for(var/name in list("l_hand","r_hand","l_arm","r_arm","l_leg","r_leg","l_foot","r_foot"))
		...
		if(current_species && (current_species.flags & HAS_COLOR))
			preview_icon.Blend(rgb(r_fur, g_fur, b_fur), ICON_ADD)
		...
		if (current_species.tail && (current_species.flags & HAS_TAIL)&& !current_species.has_color())
			var/icon/tail = new /icon ('icons/effects/species.dmi',"[current_species.tail]_s")
			preview_icon.Blend(tail, ICON_OVERLAY)
		if(current_species.tail && (current_species.flags & HAS_COLOR))
			if(current_species.tail)
				if(gender == MALE)
					var/icon/wings = new /icon ('icons/effects/species.dmi',"[current_species.tail]_m_s")
					wings.Blend(rgb(r_fur, g_fur, b_fur), ICON_ADD)
					preview_icon.Blend(wings, ICON_OVERLAY)
				else
					var/icon/wings = new /icon ('icons/effects/species.dmi',"[current_species.tail]_f_s")
					wings.Blend(rgb(r_fur, g_fur, b_fur), ICON_ADD)
					preview_icon.Blend(wings, ICON_OVERLAY)
				...
				if(ACCOUNTANT)
					...
					switch(backbag)
						...
						if(4)
							clothes_s.Blend(new /icon('icons/mob/back.dmi', "satchel"), ICON_OVERLAY)
				if(ACCOUNTANT)
					clothes_s = new /icon('icons/mob/uniform.dmi', "red_suit")
					clothes_s.Blend(new /icon('icons/mob/feet.dmi', "black"), ICON_UNDERLAY)
					if(prob(1))
						clothes_s.Blend(new /icon('icons/mob/head.dmi', "hairflower"), ICON_OVERLAY)
					switch(backbag)

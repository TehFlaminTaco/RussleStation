...
/client/Move(n, direct)
	...
	if(ispAI(mob))
		var/mob/living/silicon/pai/D = mob
		if(istype(D.current,/obj/item/device/paicard))
			return pAIMove(n,direct,mob)
	...
	if((istype(mob.loc, /turf/space)) || (mob.lastarea.has_gravity == 0))
		...
		if(istype(mob,/mob/living/simple_animal/spiderbot))
			var/mob/living/simple_animal/spiderbot/S = mob
			if(S.fly)
				mob.floatiness = 1
			else
				mob.floatiness = 0

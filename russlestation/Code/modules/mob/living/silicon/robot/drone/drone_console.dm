...
/obj/machinery/computer/drone_control/attack_robot(var/mob/user as mob)
	if(isdrone(user))
		user << "You do not have access to this console"
		return
...
/obj/machinery/computer/drone_control/attack_hand(var/mob/user as mob)
	...
	for(var/mob/living/silicon/robot/drone/D in world)
		...
		if(D.gps && !D.emagged)
			dat += "<BR>"
			for(var/obj/item/device/gps/borg/G in D)
				var/turf/pos = get_turf(G)
				dat += "<BR>Currently located in: [get_area(D)]."
				dat += "<BR>GPS Cords:([pos.x], [pos.y], [pos.z])"
				dat += "<BR>"
		else
			dat += "<BR>"
			dat += "No GPS Signal"
		...
		if(!D.emagged)
			...
			dat += "<BR>No Response from Resync/Shutdown Signal</font>"

/mob/living/silicon/robot
	...
	var/vip_triesleft = 5 //Vip custom sprite uses
	var/obj/item/borg/sight/hud = null
	var/openpanel_icon = "ov-openpanel"
...
//3 Modules can be activated at any one time.
	...
	var/obj/item/device/gps/borg/gps = null
	var/mob/living/silicon/ai/connected_ai = null
	var/obj/item/weapon/cell/cell = null
	var/old_cell = null //used in admin rejuve
	...
	var/pose
	var/rolling_state = null
	var/base_icon = null
	var/old_mind_name = null
...
/mob/living/silicon/robot/proc/setup_GPS()

	if(custom_name)
		if (!gps)
			gps = new /obj/item/device/gps/borg(src)
		gps.gpstag = "[custom_name]"
		gps.name = "GPS ([gps.gpstag])"
	else
		if (!gps)
			gps = new /obj/item/device/gps/borg(src)
		gps.gpstag = "[real_name]"
		gps.name = "GPS ([gps.gpstag])"
...
/mob/living/silicon/robot/proc/pick_module()
	...
	if(modtype == "Medical" || modtype == "Security" || modtype == "Combat" || modtype == "Surgery")
...
/mob/living/silicon/robot/proc/show_gps_location()
	if(gps)
		stat(null,"GPS Location:([x], [y], [z])")
	else
		stat(null, text("No GPS installed"))
...
/mob/living/silicon/robot/proc/updateicon()
	...
	if(opened)
		if(wiresexposed)
			overlays += "[openpanel_icon] +w"
		else if(cell)
			overlays += "[openpanel_icon] +c"
		else
			overlays += "[openpanel_icon] -c"
...
/mob/living/silicon/robot/Topic(href, href_list)
	...
	if (href_list["act"])
		...
		if(!module_state_1)
			...
			if(istype(module_state_1,/obj/item/weapon/reagent_containers/borghypo))
				processing_objects += module_state_1
			...
			if(istype(module_state_2,/obj/item/weapon/reagent_containers/borghypo))
				processing_objects += module_state_2
			...
			if(istype(module_state_3,/obj/item/weapon/reagent_containers/borghypo))
				processing_objects += module_state_3
		else
...
/mob/living/silicon/robot/verb/use_gps()
	set category = "Robot Commands"
	set name = "Use GPS"
	var/dat = ""
	if(gps.emped)
		...
		for(var/obj/item/device/gps/G in GPS_list)
			...
			if(G.emped == 1)
				dat += "<BR>[tracked_gpstag]: ERROR"
			else
				dat += "<BR>[tracked_gpstag]: [format_text(gps_area.name)] ([pos.x], [pos.y], [pos.z])"

	var/datum/browser/popup = new(usr, "GPS", name, 600, 450)

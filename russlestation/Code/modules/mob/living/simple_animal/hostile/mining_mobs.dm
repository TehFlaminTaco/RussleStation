...
/mob/living/simple_animal/hostile/asteroid/basilisk/Die()
	...
	for(counter=0, counter<2, counter++)
		...
		D.layer = 4.1
	var/obj/item/weapon/card/mining_point_card/M = new/obj/item/weapon/card/mining_point_card(src.loc)
	M.layer = 4.2
	M.points = 1000
...
/mob/living/simple_animal/hostile/asteroid/goldgrub/proc/Reward()
	...
	if(!ore_eaten || ore_types_eaten.len == 0)
		var/counter
		for(counter=0, counter<2, counter++)
		var/obj/item/weapon/ore/gold/D = new /obj/item/weapon/ore/gold(src.loc)
		D.layer = 4.1
		return
...
/mob/living/simple_animal/hostile/asteroid/goliath/OpenFire(var/the_target)
	if(client)
		var/target = the_target
		if(istype(target, /mob/living))
			var/mob/living/D = target
			visible_message("<span class='warning'>The [src.name] digs its tentacles under [D.name]!</span>")
		else
			visible_message("<span class='warning'>The [src.name] digs its tentacles under [target]!</span>")
		var/tturf = get_turf(target)
		new /obj/effect/goliath_tentacle/original(tturf)
		ranged_cooldown = ranged_cooldown_cap
		client_cooldown = client_cooldown_cap
		return
	else
...
/mob/living/simple_animal/hostile/ClickOn(var/atom/A)
	...
	if(A.Adjacent(src))
		if(istype(A, /mob))
			A.attack_animal(src)
			return
		else if(istype(A, /obj/machinery/bot))
			A.attack_animal(src)
			return
			...
			return
	if(ranged)
		var/TargetDistance = get_dist(src,A)
		if(TargetDistance >= 2 && client_cooldown <= 0)//But make sure they're a tile away at least, and our range attack is off cooldown
			OpenFire(A)
		else if(client_cooldown >=1)
			src << "Ranged attack on cooldown"
	if(istype(src, /mob/living/simple_animal/hostile/asteroid/basilisk))
		if(istype(A,/mob/living) && client_cooldown <= 0)
			var/mob/living/L = A
			if(L.bodytemperature > 261)
				L.bodytemperature = 261
				visible_message("<span class='danger'>The [src.name]'s stare chills [L.name] to the bone!</span>")

/mob/living/simple_animal/hostile/DblClickOn(var/atom/A)
	return
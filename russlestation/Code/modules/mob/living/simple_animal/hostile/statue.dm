...
/mob/living/simple_animal/hostile/statue
	...
	search_objects = 1 // So that it can see thro\ugh walls
...
/mob/living/simple_animal/hostile/statue/New()
	...
	spell_list += new /obj/effect/proc_holder/spell/aoe_turf/statueaffy(src)
...
/mob/living/simple_animal/hostile/statue/Life()
	...
	if(!client && target) // If we have a target and we're AI controlled
		...
		if(watching && watching != target && CanAttack(watching))
			// This one is closer.
	...
	for(var/mob/living/L in living_mob_list)
		...
		if(T && T in targets)
			...
			if(issilicon(L))
				L.Weaken(4)
	return
/obj/effect/proc_holder/spell/aoe_turf/statueaffy
	name = "Stone Glare"
	desc = "Your prey will be turned into a statue for you to advance on them."
...
/obj/effect/proc_holder/spell/aoe_turf/statueaffy/cast(list/targets)
	for(var/mob/living/L in living_mob_list)
		var/turf/T = get_turf(L.loc)
		if(T && T in targets)

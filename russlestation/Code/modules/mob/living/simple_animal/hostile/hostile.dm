...
/mob/living/simple_animal/hostile/Stat()
	...
	if(client)
		if (client.statpanel == "Status")
			if(ranged)
				if(client_cooldown >=1)
					stat("Ranged Cooldown:[client_cooldown]")
				else
					stat("Ranged Attack Ready")
...
/mob/living/simple_animal/hostile/proc/ListTargets()//Step 1, find out what we can see
	...
	if(search_objects < 2)
		...
		for(var/mob/living/G in Mobs)
			L.Add(G)
		for(var/mob/living/D in friends)
			L.Remove(D)
...
/mob/living/simple_animal/hostile/proc/MoveToTarget()//Step 5, handle movement between us and our target
	...
	if(friends)
		if(!target || SA_attackable(target))
			Goto(friends,move_to_delay,minimum_distance)
	if(!target || SA_attackable(target))
		LoseTarget()

/mob/living/carbon/human/emote(var/act,var/m_type=1,var/message = null)
	...
	if(src.stat == 2.0 && (act != "deathgasp"))
		return
	if(act == "giggle" || act == "cough" || act == "drool" || act == "twitch" || act == "shiver" || act == "sneeze" || act== "scream" || act == "collapse" || act == "groan")
		gspam = 1
	switch(act)
		if ("airguitar")
			if (!src.restrained())
		...
		for(var/mob/M in dead_mob_list)
			...
			if(M.stat == DEAD && (M.client.prefs.toggles & CHAT_GHOSTSIGHT) && !(M in viewers(src,null)))
				if(istype(M, /mob/dead/observer))
					...
					if((O.aghost || O.started_as_observer) && !gspam)
						O.show_message(message)
					else if((!O.aghost || !O.started_as_observer) && !gspam)
						O.show_message(message)
				else
		...
		if (m_type & 1)
			for (var/mob/O in get_mobs_in_view(world.view,src))
				if(istype(O, /mob/dead/observer))
					var/mob/dead/observer/Ob = O
					if((Ob.aghost || Ob.started_as_observer) && !gspam)
						Ob.show_message(message, m_type)
					else if((!Ob.aghost || !Ob.started_as_observer) && !gspam)
						Ob.show_message(message, m_type)
				else

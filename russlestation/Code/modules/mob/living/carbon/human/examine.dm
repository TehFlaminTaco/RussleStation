/mob/living/carbon/human/examine()
	...
	if(wear_suit)
		...
		skipbody = wear_suit.flags_inv & HIDEBODY
	...
	if(head)
		...
		skipbody |= head.flags_inv & HIDEBODY
	...
	if( (skipjumpsuit && skipface) || skipbody) //big suits/masks/helmets make it hard to tell their gender
		t_He = "They"
		t_his = "their"
		t_him = "them"
		t_has = "have"
	...
	//uniform
	if(w_uniform && !skipjumpsuit && !skipbody)
		//Ties
		var/tie_msg
	...
	//mask
	if(wear_mask && !skipmask && !skipbody)
	...
	//left ear
	if(l_ear && !skipears && !skipbody)
		msg += "[t_He] [t_has] \icon[l_ear] \a [l_ear] on [t_his] left ear.\n"

	//right ear
	if(r_ear && !skipears && !skipbody)
...
/proc/hasHUD(mob/M as mob, hudtype)
	if(istype(M, /mob/living/carbon/human))
		...
		switch(hudtype)
			if("security")
				if(R.modtype == "Security" || R.modtype == "Combat")
					return 1
				return 0
			if("medical")
				if(R.modtype == "Medical" || R.modtype == "Surgery")
					return 1
				return 0
			else
				return 0

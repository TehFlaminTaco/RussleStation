				...
				spawn( 0 )
					...
					/*switch(rand(1, 3))
						if(1)
							emote("twitch")
						if(2 to 3)
							say("[prob(50) ? ";" : ""][pick("SHIT", "PISS", "FUCK", "CUNT", "COCKSUCKER", "MOTHERFUCKER", "TITS")]")*/ //Nope. ~Aztec
					var/old_x = pixel_x
					var/old_y = pixel_y
					pixel_x += rand(-2,2)
					pixel_y += rand(-1,1)
...
//					log_debug("[src] : Exhaling some viruses")
	...
	proc/handle_environment(datum/gas_mixture/environment)
		...
		if(status_flags & GODMODE)	return 1	//godmode
		if(istype(loc, /obj/structure/transit_tube_pod)) return 1
		...
		if(adjusted_pressure >= species.hazard_high_pressure)
			...
			pressure_alert = 2
		else if((adjusted_pressure < species.hazard_high_pressure) && (adjusted_pressure >= species.warning_high_pressure))
			pressure_alert = 1
		else if((adjusted_pressure < species.warning_high_pressure) && (adjusted_pressure > species.warning_low_pressure))
			pressure_alert = 0
		else if((adjusted_pressure < species.warning_low_pressure) && (adjusted_pressure > species.hazard_low_pressure))
			pressure_alert = -1
...
*/
	...
	proc/handle_regular_status_updates()
		if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
			...
			if(hallucination)
//				if(species && (species.flags & IS_SYNTHETIC))	//WHY WOULD YOU MAKE IT THAT MACHINES NEVER HEAL FROM HALLUCINATIONS?! ~Aztec
//					return
				if(hallucination >= 20)
					if(prob(3))
						fake_attack(src)
...
*/
	...
	proc/handle_regular_hud_updates()
		...
		client.screen.Remove(global_hud.blurry, global_hud.druggy, global_hud.vimpaired, global_hud.darkMask, global_hud.nvg, global_hud.therm)
				...
				if(istype(wear_mask, /obj/item/clothing/mask/gas/voice/space_ninja))
					...
					switch(O.mode)
						...
						if(2)
							...
							see_in_dark = 7
							if(!druggy)		see_invisible = SEE_INVISIBLE_MINIMUM
							var/obj/item/clothing/glasses/thermal/glasses = new/obj/item/clothing/glasses/thermal(src.contents)
							var/obj/item/clothing/glasses/thermal/D = glasses
							if(D.hud)		D.hud.process_hud(src)
							client.screen += global_hud.therm
						if(3)
							sight |= SEE_TURFS
							see_in_dark = 7
							if(!druggy)		see_invisible = SEE_INVISIBLE_MINIMUM
	...
	// The purpose of this is to stop xray and w/e from preventing you from using huds -- Love, Doohl
				...
				if(istype(glasses, /obj/item/clothing/glasses/thermal))
					var/obj/item/clothing/glasses/thermal/O = glasses
					if(O.hud)		O.hud.process_hud(src)
					if(!druggy)		see_invisible = SEE_INVISIBLE_MINIMUM
					see_in_dark = 7
					client.screen += global_hud.therm
...
//				else									rest.icon_state = "rest0"
	...
	proc/handle_ninja()
		if(isninja(src))
			if(hud_used)
				if(!hud_used.ninja_cell_display)
					hud_used.ninja_hud()
					//hud_used.human_hud(hud_used.ui_style)
				hud_used.ninja_cell_display.overlays.Cut()
				var/image/A = text2icon("Ec",23,-9)
				var/percent =num2text(round((wear_suit:cell:charge/(wear_suit:cell:maxcharge/100))))
				var/image/B = text2icon("[percent]%",23,-17)
				hud_used.ninja_cell_display.overlays += A
				hud_used.ninja_cell_display.overlays += B
...
/mob/living/carbon/human/proc/handle_hud_list()
	...
	if(hud_updateflag & 1 << THERMAL_HUD)
		var/image/holder = hud_list[THERMAL_HUD]
		if(stat !=2)
			holder.icon_state = "hud_thermal"
		else
			holder.icon_state = null


		hud_list[THERMAL_HUD] = holder
	...
	if(hud_updateflag & 1 << SPECIALROLE_HUD)
		...
		if(mind)
			switch(mind.special_role)
				if("traitor","Syndicate","Syndicate Assimilation Borg")

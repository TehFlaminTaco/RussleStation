	...
	for(var/bp in body_parts)
		...
		if(bp && istype(bp ,/obj/item/clothing))
			...
			if(C.body_parts_covered & HEAD)
				return 1
 	return 0
/mob/living/carbon/human/proc/check_ear_coverage()
	if(head && istype(head,/obj/item/clothing/))
		var/obj/item/clothing/H = head
		if(H.flags_inv & HIDEEARS)
			return 1
	if(wear_mask && istype(wear_mask,/obj/item/clothing/))
		var/obj/item/clothing/M = wear_mask
		if(M.flags_inv & HIDEEARS)
			return 1
	if(l_ear && istype(l_ear,/obj/item/clothing/))
		var/obj/item/clothing/le = l_ear
		if(le.flags_inv & HIDEEARS)
			return 1
	if(r_ear && istype(r_ear,/obj/item/clothing/))
		var/obj/item/clothing/re = r_ear
		if(re.flags_inv & HIDEEARS)
			return 1
 	return 0

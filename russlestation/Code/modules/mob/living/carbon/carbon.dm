...
/mob/living/carbon/proc/borer_speak()
	set category = "Borer"
	set name = "Cortical link"
	set desc = "Speak to your fellow borers"
	var/msg = sanitize(input("Message:", "Cortical link") as text|null)
	var/mob/living/simple_animal/borer/myborer = src.has_brain_worms()
	if(!myborer)return
	if(!msg)return
	if(msg)
		for(var/mob/M in player_list)
			if(M.mind && (istype(M, /mob/living/simple_animal/borer)))
				M << "<font color =#00b273><i>Cortical link, <b>[myborer.truename]:</b> [msg]</i></font>"
				...
				if(borer)
					if(borer.controlling)
						C << "<font color =#00b273><i>Cortical link, <b>[myborer.truename]:</b> [msg]</i></font>"
			else if(M.mind && istype(M, /mob/dead/observer))
				var/mob/dead/observer/obs = M
				if(obs.client.holder || obs.has_enabled_antagHUD)
					obs << "<font color =#00b273><i>Cortical link, <b>[myborer.truename]:</b> [msg]</i></font>"

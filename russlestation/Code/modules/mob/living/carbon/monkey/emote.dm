	...
	switch(act)
		...
		if("roar")
			if (!muzzled)
				...
				m_type = 2
		if("tail")
			message = "<B>[src.name]</B> waves his tail."
			m_type = 1
		if("gasp")
		...
		if("paw")
			if (!src.restrained())
				message = "<B>[src.name]</B> flails his paw."
				m_type = 1
		if("scretch")
			if (!muzzled)
		...
		if("gnarl")
			if (!muzzled)
				message = "<B>[src.name]</B> gnarls and shows his teeth.."
				m_type = 2
		if("jump")
		...
		if("collapse")
			...
			m_type = 2
		if("deathgasp")
			message = "<b>[src.name]</b> lets out a faint chimper as it collapses and stops moving..."
			m_type = 1
		if("help")
			var/text = "choke, "
			if(istype(src,/mob/living/carbon/monkey/diona))

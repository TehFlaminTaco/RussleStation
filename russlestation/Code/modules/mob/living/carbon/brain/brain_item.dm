...
/obj/item/device/mmi/posibrain/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	if(!istype(M, /mob))
		return
	...
	if(M:brain_op_stage == 4.0)
		for(var/mob/O in viewers(M, null))
			if(O == (user || M))
				continue
			if(M == user)
				O.show_message(text("\red [user] inserts [src] into his head and reactivate its body!"), 1)
			else
				O.show_message(text("\red [M] has [src] inserted into his head and has its body reactivate by [user]."), 1)

		if(M != user)
			M << "\red [user] inserts [src] into your head and reactivate your body!"
			user << "\red You insert [src] into [M]'s head and reactivate its body!"
		else
			user << "\red You insert [src] into your head and reactivate its body!"

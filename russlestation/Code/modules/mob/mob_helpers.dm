...
/proc/issynth(A)
	if(istype(A, /mob/living/carbon/human))
		var/mob/living/carbon/human/S = A
		if(S.species && (S.species.flags & IS_SYNTHETIC))
			return 1
		else
			return 0
	return 0
...
proc/isninja(A)
	if(istype(A,/mob/living/carbon/human))
		...
		if(H.wear_suit)
			if (istype(H.wear_suit, /obj/item/clothing/suit/space/space_ninja)&&H.wear_suit:s_initialized)
				return 1
			else
				return 0
		else

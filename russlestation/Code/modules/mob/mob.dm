...
/mob/verb/abandon_mob()
	...
	if (ticker.mode.name == "meteor" || ticker.mode.name == "epidemic") //BS12 EDIT
		...
		if (client)
			if (client.time_died_as_mouse>0)
				deathtime = world.time - max(src.timeofdeath, client.time_died_as_mouse)

		if (src.aghost==1)
			usr << "\red You cannot respawn whilst admin ghosted."
			return
...
/mob/proc/start_pulling(var/atom/movable/AM)
	...
	if(istype(AM, /obj/structure/stool/bed/chair/wheelchair))
		var/obj/structure/stool/bed/chair/wheelchair/WC = AM
		WC.buckled_mob.pulledby = src
		WC.handle_rotation()
...
/mob/proc/floaty_process()
	...
	while(floatiness)
		if(istype(src,/mob/living/simple_animal/spiderbot))
			pixel_y = 10+(3 * cos(7 * world.time)) //May need some tweaking
		else

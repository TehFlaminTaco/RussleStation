#define SAVEFILE_VERSION_MIN	16
#define SAVEFILE_VERSION_MAX	19 //update 3/4
...
/datum/preferences/proc/savefile_update()
	if(savefile_version < 16)	//lazily delete everything + additional files so they can be saved in the new format
...
/datum/preferences/proc/load_preferences()
	...
	Atoggles		= sanitize_integer(Atoggles, 0, 65535, initial(Atoggles))
...
/datum/preferences/proc/getspecies()
	if(!path)				return 0
	if(!fexists(path))		return 0
	var/savefile/S = new /savefile(path)
	if(!S)					return 0
	var/checkspecies
	S["species"]			>> checkspecies

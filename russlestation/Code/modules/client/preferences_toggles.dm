...
/client/verb/toggle_tracking()
	set name = "Enable/Disable Ghost Tracking"
	set category = "Preferences"
	set desc = ".Toggle between seeing all player (follow)links while observer/ghost"
	prefs.toggles ^= CHAT_TRACK
	src << "As a ghost, you will now [(prefs.toggles & CHAT_TRACK) ? "see all (follow) link while dead" : "not see any (follow) link while dead"]."
	prefs.save_preferences()
	feedback_add_details("admin_verb","TRACK")
...
/client/proc/toggle_hear_radio()
	...
	if(!holder) return
	prefs.Atoggles ^= CHAT_RADIO
	prefs.save_preferences()
	usr << "You will [(prefs.Atoggles & CHAT_RADIO) ? "now" : "no longer"] see radio chatter from radios or speakers"
...
/client/proc/toggleprayers()
	...
	prefs.Atoggles ^= CHAT_PRAYER
	prefs.save_preferences()
	src << "You will [(prefs.Atoggles & CHAT_PRAYER) ? "now" : "no longer"] see prayerchat."

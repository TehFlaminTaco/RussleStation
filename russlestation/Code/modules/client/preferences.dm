...
//some autodetection here.
	...
	"diona" = 1,                                         // 12
	"vampire" = IS_MODE_COMPILED("vampire")			 // 12
)
...
datum/preferences
	...
	var/Atoggles = ADMIN_TOGGLES_DEFAULT
...
/datum/preferences
	...
	proc/GetSkillClass(points)
		...
		switch(points)
			...
			if(24 to 1000)
				return "God"
	proc/GetPrefSpecies(mob/user)
		checkspecies = species
		user.mind.checkspecies = species
		return
	...
	proc/ShowChoices(mob/user)
		...
		dat += "<br><br>"
		if((species == "Aviskree")||(species == "Machine")||(species == "Skrell")||(species == "Unathi"))
			dat += "<br><b>Skin Color(WIP)</b><br>"
			dat += "<a href='?_src_=prefs;preference=fur;task=input'>Change Color</a> <font face='fixedsys' size='3' color='#[num2hex(r_fur, 2)][num2hex(g_fur, 2)][num2hex(b_fur, 2)]'><table style='display:inline;' bgcolor='#[num2hex(r_fur, 2)][num2hex(g_fur, 2)][num2hex(b_fur)]'><tr><td>__</td></tr></table></font> "
			dat += "<br><br>"
		...
		user << browse(dat, "window=preferences;size=560x830")

	proc/SetChoices(mob/user, limit = 16, list/splitJobs = list("Chief Medical Officer","Chief Engineer","Head of Security"), width = 790, height = 695)
		if(!job_master)
			return
		...
		for(var/datum/job/job in job_master.occupations)
			...
			if((index >= limit) || (job.title in splitJobs))
				if((index < limit) && (lastJob != null))
					...
					for(var/i = 0, i < (limit - index), i += 1)
						HTML += "<tr bgcolor='[lastJob.selection_color]'><td width='60%' align='right'></td></tr>"
				HTML += "</table></td><td width='20%' valign='top'><table width='100%' cellpadding='1' cellspacing='0'>"
				index = 0

			HTML += "<tr bgcolor='[job.selection_color]'><td width='60%' align='right' valign='top'>"
			var/rank = job.title
			lastJob = job
			...
			if(rank == "Assistant")//Assistant is special
				if(job_civilian_low & ASSISTANT)
					HTML += " <font color=green>\[Yes]</font></a></td></tr><tr bgcolor='[lastJob.selection_color]'><td width='60%' align='center'><a>&nbsp</a></td><td><a href=\"byond://?src=\ref[user];preference=job;task=alt_title;job=\ref[job]\">\[[GetPlayerAltTitle(job)]\]</a></td></tr>"
				else
					HTML += " <font color=red>\[No]</font></a></td></tr><tr bgcolor='[lastJob.selection_color]'><td width='60%' align='center'><a>&nbsp</a></td><td><a href=\"byond://?src=\ref[user];preference=job;task=alt_title;job=\ref[job]\">\[[GetPlayerAltTitle(job)]\]</a></td></tr>"
				HTML += "</a></td></tr>"
				continue
			...
			if("random")
				switch(href_list["preference"])
					...
					if("hair")
						...
						b_hair = rand(0,255)
					if("fur")
						r_fur = (rand(31,255)-30)
						g_fur = (rand(31,255)-30)
						b_fur = (rand(31,255)-30)
					if("h_style")
			...
			if("input")
				switch(href_list["preference"])
					...
					if("species")
						...
						var/list/new_vip_species = list("Human")
						...
						if(config.usealienwhitelist) //If we're using the whitelist, make sure to check it!
							...
							new_vip_species = vipspecies

						if(vip_check_rights(V_SUPER)||check_rights(R_ADMIN|R_MOD))
							species = input("Please select a species", "Character Generation", null) in new_vip_species
						else
					...
					if("hair")
						if(species == "Human" || species == "Unathi"|| species == "Aviskree"|| species == "Vox" || species == "kidan"|| species == "Skrell")
					...
					if("fur")
						if(species == "Aviskree"||species == "Machine"|| species == "Skrell" || species == "Unathi")
							var/new_fur = input(user, "Choose your character's fur colour:", "Character Preference") as color|null
							if(new_fur)
								r_fur = hex2num(copytext(new_fur, 2, 4))-30
								if(r_fur<=0)
									r_fur = 1
								g_fur = hex2num(copytext(new_fur, 4, 6))-30
								if(g_fur<=0)
									g_fur = 1
								b_fur = hex2num(copytext(new_fur, 6, 8))-30
								if(b_fur<=0)
									b_fur = 1
	...
	proc/copy_to(mob/living/carbon/human/character, safety = 0)
		...
		character.r_fur = r_fur
		character.g_fur = g_fur
		character.b_fur = b_fur

...
/client/New(TopicData)
	...
	if(IsGuestKey(key))
		...
		del(src)
		return
/*	if(global_clients >= max_clients)
		if(!(vipholder||holder))
			alert(src,"This server has reached its maximun number of players.")
			return null */
	...
	if(prefs.readrules != read_the_rules)
		...
		src << "\blue<b>Welcome [ckey] to the server if this is your first time playing with us please read our rules!<br>If not the rules may have been updated or Preferences Reset.<br>Please click the red rules button in the top right of your screen to disable this meassage</b>"
...
/client/proc/log_client_to_db()
	...
	if(sql_id)
		...
		var/DBQuery/query_update = dbcon.NewQuery("UPDATE erro_player SET lastseen = Now(), ip = '[sql_ip]', computerid = '[sql_computerid]', lastviprank = '[sql_vip_rank]', lastadminrank = '[sql_admin_rank]' WHERE id = [sql_id]")
		query_update.Execute()
	else
		//New player!! Need to insert all the stuff
		var/DBQuery/query_insert = dbcon.NewQuery("INSERT INTO erro_player (id, ckey, firstseen, lastseen, ip, computerid, lastadminrank, lastviprank) VALUES (null, '[sql_ckey]', Now(), Now(), '[sql_ip]', '[sql_computerid]', '[sql_admin_rank]', '[sql_vip_rank]')")
...
/*
/client/proc/preload_hailer()
	var/list/hailer_files = flist(HAILER_PATH)
	for(var/file in hailer_files)
		//src << "Downloading [file]"
		var/sound/S = sound("[HAILER_PATH][file]")
		src << getFiles(S)
*/
	...
	if(istype(M, /mob/living) && M.stat != DEAD)
		...
		switch(randomize)
			...
			if("xeno")
				...
				new_mob.universal_speak = 1*/
			if("animal")
				var/animal = pick("parrot","corgi","crab","cat","carp","bear","mushroom","tomato","mouse","chicken","cow","lizard","chick")
				switch(animal)
...
//						if("pug")		new_mob = new /mob/living/simple_animal/pug(M.loc)
			...
			if("human")
				new_mob = new /mob/living/carbon/human(M.loc,pick("Unathi","Tajaran","Human","Aviskree","Skrell","Diona"))
				if(M.gender == MALE)
					new_mob.gender = MALE

...
/obj/item/weapon/gun/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if (M == user && user.zone_sel.selecting == "mouth" && !mouthshoot)
		mouthshoot = 1
		if(issynth(M))
			M.visible_message("\red [user] sticks their gun to their head, ready to pull the trigger...")
		else
			M.visible_message("\red [user] sticks their gun to their head, ready to pull the trigger...")
		if(!do_after(user, 40))
		...
		if (load_into_chamber())
			...
			if (in_chamber.damage_type != HALLOSS)
				if(issynth(M))
					user.apply_damage(in_chamber.damage*2.5, in_chamber.damage_type, "head", used_weapon = "Point blank shot in the head with \a [in_chamber]")
				else

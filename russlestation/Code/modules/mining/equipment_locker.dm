...
/obj/machinery/mineral/ore_redemption/proc/process_sheet(obj/item/weapon/ore/O)
	...
	if(processed_sheet)
		...
		if(mat.stored == stack_amt) //Get rid of excessive stackage
			var/obj/item/stack/sheet/out = new mat.sheettype
			out.amount = mat.stored
			out.loc = output.loc
			mat.stored = 0
...
/obj/machinery/mineral/ore_redemption/interact(mob/user)
	...
	user << browse("[dat]", "window=Ore_Redemption_machine;size=420x680")
...
/obj/machinery/mineral/ore_redemption/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/card/id))
		var/obj/item/weapon/card/id/D = usr.get_active_hand()
		if(istype(D))
			usr.drop_item()
			D.loc = src
			inserted_id = D
...
/obj/machinery/mineral/equipment_locker
	...
	var/list/prize_list = list(
		...
		new /datum/data/mining_equipment("Drill",				/obj/item/weapon/pickaxe/drill,										200),
		...
		new /datum/data/mining_equipment("Kinetic accelerator", /obj/item/weapon/gun/energy/kinetic_accelerator,                  3000),
...
/obj/machinery/mineral/equipment_locker/interact(mob/user)
	...
	user << browse("[dat]", "window=mining_equipment_locker;size=300x620")
	return
	...
	if(href_list["purchase"])
		if(istype(inserted_id))
			...
			if(prize.cost > inserted_id.mining_points)
				...
				new prize.equipment_path(usr.loc)
	updateUsrDialog()
	return
		...
		spawn(50)
			...
			if(creator)
				for(var/mob/living/L in src.loc)
					log_admin("[creator] used a resonator field")
...
/mob/living/simple_animal/hostile/mining_drone/ListTargets()
	var/list/L = list()
	if(search_objects == 2)
		var/list/Mobs = hearers(src, idle_vision_range)
		for(var/mob/living/G in Mobs)
			L.Add(G)
			if(istype(G, /mob/living/carbon/human))
				L.Remove(G)//don't attack humans!
	L.Remove(src)//So we don't suicide because we listed ourselves as a target!

	if(search_objects)
		var/list/Objects = oview(idle_vision_range, src)
		for(var/obj/O in Objects)
			L.Add(O)
	else
		for(var/obj/mecha/M in mechas_list)
			if(get_dist(M, src) <= idle_vision_range && can_see(src, M, idle_vision_range))
				L += M
	return L
/mob/living/simple_animal/hostile/mining_drone/FindTarget()
	. = ..()
	if(.)
		emote("flys over towards [.]")

/mob/living/simple_animal/hostile/mining_drone/AttackingTarget()
	if(istype(target.type, /obj/item/weapon/ore))
		CollectOre()
		return
	..()
...
/obj/item/device/mining_scanner/attack_self(mob/user)
	...
	if(on)
		...
		user << "<span class='info'>You turn the [src] on!</span>"
	else
		user <<"<span class='info'>You turn the [src] off!</span>"

/obj/item/device/mining_scanner/process()
	if(!on)
		processing_objects.Remove(src)
		return null
	if(on)
		if(istype(src.loc, /mob/living))
			var/mob/living/D = src.loc
			if(!D.client)
				return
			if(!cooldown)
				cooldown = 1
				...
				for(M in range(7, D))
					if(M.scan_state)
						L += M
				if(!L.len)
					D << "<span class='info'>[src] reports that nothing was detected nearby and turns itself off.</span>"
					on = !on
					return
				else
					for(M in L)

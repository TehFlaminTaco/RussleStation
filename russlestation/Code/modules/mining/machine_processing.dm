		...
		for (i = 0; i < 10; i++)
			...
			/*
				if (selected == ORE_PROC_GLASS + ORE_PROC_PHORON)
					if (ore_glass > 0 && ore_phoron > 0)
						ore_glass--;
						ore_phoron--;
						new /obj/item/stack/sheet/glass/phoronglass(output.loc)
					else
						on = 0
					continue
				if (selected == ORE_PROC_GLASS + ORE_PROC_IRON + ORE_PROC_PHORON)
					if (ore_glass > 0 && ore_phoron > 0 && ore_iron > 0)
						ore_glass--;
						ore_iron--;
						ore_phoron--;
						new /obj/item/stack/sheet/glass/phoronrglass(output.loc)
					else
						on = 0
					continue
			*/

...
/turf/simulated/mineral //wall piece
	...
	var/scan_state = null
	var/det_time = null //Countdown till explosion, but also rewards the player for how close you were to detonation when you defuse it
	var/stage = null //How far into the lifecycle of gibtonite we are, 0 is untouched, 1 is active and attempting to detonate, 2 is benign and ready for extraction
	...
	proc/UpdateMineral()
		...
		switch(mineral.display_name)
			if("Iron")
				name = "\improper Rock"
				icon_state = "rock"
				scan_state = "rock_Iron[rand(1,3)]"
			if("Phoron")
				name = "\improper Rock"
				icon_state = "rock"
				scan_state = "rock_Phoron[rand(1,3)]"
			if("Gibtonite")
				name = "\improper Rock"
				icon_state = "rock"
				scan_state = "rock_Diamond"
				det_time = rand(8,10) //Countdown till explosion, but also rewards the player for how close you were to detonation when you defuse it
				stage = 0 //How far into the lifecycle of gibtonite we are, 0 is untouched, 1 is active and attempting to detonate, 2 is benign and ready for extraction
			if("Cave")
				name = "\improper Rock"
				icon_state = "rock"
				scan_state = "cave"
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if (istype(W, /obj/item/weapon/pickaxe))
			...
			if(istype(src,/turf/simulated/mineral))
				var/turf/simulated/mineral/R = src
				if(istype(R.mineral, /mineral/gibtonite)&& R.stage == 0)
					R.activated_ckey = "[user.ckey]"
					R.activated_name = "[user.name]"
					R.explosive_reaction()
					return
			if(istype(src,/turf/simulated/mineral))
				var/turf/simulated/mineral/R = src
				if(istype(R.mineral, /mineral/cave))
					src.ChangeTurf(/turf/simulated/floor/plating/airless/asteroid/cave)
					var/turf/simulated/floor/plating/airless/asteroid/cave/C = src
					var/ex_cave_dir = 0
					if(usr.dir == 4)
						ex_cave_dir = 8
					if(usr.dir == 1)
					...
					if(usr.dir == 8)
						ex_cave_dir = 4
					C.spawn_cave(src.loc,rand(25, 50),0,ex_cave_dir)
					usr << "\blue You find an entrance to a cave!"
					return
		...
		if(istype(src,/turf/simulated/mineral))
			var/turf/simulated/mineral/R = src
			if(istype(R.mineral, /mineral/gibtonite))
				if(istype(W, /obj/item/device/mining_scanner) && stage == 1)
	...
	proc/GetDrilled(var/artifact_fail = 0)
		if(istype(src,/turf/simulated/mineral))
			...
			if(istype(R.mineral, /mineral/gibtonite))
				...
				if(R.stage == 0 && R.mineral.result_amount >= 1) //Gibtonite deposit is activated
					playsound(src,'sound/effects/hit_on_shattered_glass.ogg',50,1)
					explosive_reaction()
					return
				if(R.stage == 1 && R.mineral.result_amount >= 1) //Gibtonite deposit goes kaboom
					var/turf/bombturf = get_turf(src)
					R.mineral= null
					explosion(bombturf,1,2,5, adminlog = 0)
					return
				if(R.stage == 2) //Gibtonite deposit is now benign and extractable. Depending on how close you were to it blowing up before defusing, you get better quality ore.
					var/obj/item/weapon/twohanded/required/gibtonite/G = new /obj/item/weapon/twohanded/required/gibtonite/(src)
					if(R.det_time <= 2)
						G.quality = 3
						G.icon_state = "Gibtonite ore 3"
					if(R.det_time >= 3 && det_time <= 6)
						G.quality = 2
			...
			if(istype(R.mineral, /mineral/cave))
				src.ChangeTurf(/turf/simulated/floor/plating/airless/asteroid/cave)
				var/turf/simulated/floor/plating/airless/asteroid/cave/C = src
				C.spawn_cave(src.loc,rand(25, 50),0,dir)
				return
...
/turf/simulated/mineral/random
	name = "Rock"
	var/mineralSpawnChanceList = list("Uranium" = 5, "Iron" = 50, "Diamond" = 3, "Gold" = 5, "Silver" = 5, "Phoron" = 25, "Gibtonite" = 5, "Cave" = 1)//Currently, Adamantine won't spawn as it has no uses. -Durandan
...
/turf/simulated/mineral/random/high_chance
	mineralChance = 25
	mineralSpawnChanceList = list("Uranium" = 10, "Iron" = 30, "Diamond" = 3, "Gold" = 10, "Silver" = 10, "Phoron" = 25, "Gibtonite" = 2, "Cave" = 1)

/turf/simulated/mineral/random/high_chance_clown
	mineralChance = 40
	mineralSpawnChanceList = list("Uranium" = 10, "Iron" = 30, "Diamond" = 2, "Gold" = 5, "Silver" = 5, "Phoron" = 25, "Clown" = 15)
...
/turf/simulated/mineral/proc/explosive_reaction()
	if(stage == 0)
		icon_state = "rock_Gibtonite_active"
		name = "Gibtonite deposit"
		desc = "An active gibtonite reserve. Run!"
		stage = 1
		visible_message("<h2><span class='warning'><B>There was gibtonite inside! It's going to explode! Try to defuse the reaction with a mining scanner!</span></B></h2>")
		var/turf/bombturf = get_turf(src)
...
/turf/simulated/mineral/proc/countdown()
	spawn(0)
		if(stage)
			while(src.stage == 1 && src.det_time > 0 && src.mineral.result_amount >= 1)
				src.det_time--
				sleep(5)
			if(src.stage == 1 && src.det_time <= 0 && src.mineral.result_amount >= 1)
				var/turf/bombturf = get_turf(src)
				src.mineral = null
				explosion(bombturf,1,3,5, adminlog = 1)
			if(stage == 0 || stage == 2)
				return

/turf/simulated/mineral/proc/defuse()
	if(stage == 1)
		...
		if(det_time < 0)
			det_time = 0
		visible_message("<span class='notice'>The chain reaction was stopped! The gibtonite had [src.det_time] seconds left till the explosion!</span>")


/turf/simulated/floor/plating/airless/asteroid/gibtonite_remains
	var/det_time = 0
	var/stage = 0
	icon_state = "gibtonite_remains"
...
/turf/simulated/floor/plating/airless/asteroid/cave/proc/spawn_cave(loc, var/length, var/go_backwards = 1, var/exclude_dir = -1)
...
/turf/simulated/floor/plating/airless/asteroid/cave/proc/make_tunnel(var/dir)
	...
	var/turf/simulated/mineral/random/tunnel2
	...
	for(var/i = 0; i < length; i++)
		...
		for(var/edge_angle in L)
			...
			var/turf/simulated/mineral/random/edge2 = get_step(tunnel2, angle2dir(dir2angle(dir) + edge_angle))
			if(istype(edge))
				SpawnFloor(edge)
			if(istype(edge2))
				SpawnFloor(edge2)
		...
		tunnel2 = get_step(tunnel2,dir)

		if(istype(tunnel))
			// Small chance to have forks in our tunnel; otherwise dig our tunnel.
			if(i > 3 && prob(20))
				tunnel.ChangeTurf(/turf/simulated/floor/plating/airless/asteroid/cave)
				var/turf/simulated/floor/plating/airless/asteroid/cave/C = src
				C.spawn_cave(tunnel.loc, rand(10, 15), 0, dir)
			else
				SpawnFloor(tunnel)
		else if(istype(tunnel2))
			if(i > 3 && prob(20))
				tunnel2.ChangeTurf(/turf/simulated/floor/plating/airless/asteroid/cave)
				var/turf/simulated/floor/plating/airless/asteroid/cave/C = src
				C.spawn_cave(tunnel2.loc, rand(10, 15), 0, dir)
			else
				SpawnFloor(tunnel2)
...
/turf/simulated/floor/plating/airless/asteroid/cave/proc/SpawnFloor(var/turf/T)
	...
	if(istype(T,/turf/simulated/mineral))
		var/turf/simulated/mineral/G = T
		if(G.mineral)
			if(istype(G.mineral,/mineral/gibtonite))
				G.explosive_reaction()
			else if(istype(G.mineral,/mineral/cave))
				src.ChangeTurf(/turf/simulated/floor/plating/airless/asteroid/cave)
				var/turf/simulated/floor/plating/airless/asteroid/cave/C = G
				C.spawn_cave(src.loc,rand(25, 50),0,dir)
			else
				G.DropMineral()

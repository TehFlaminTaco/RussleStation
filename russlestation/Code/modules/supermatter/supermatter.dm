...
//        var/config_power_reduction_per_tick = 0.5
	...
	shard //Small subtype, less efficient and more sensitive, but less boom.
		...
		warning_point = 50
		emergency_point = 500
		explosion_point = 1200

		gasefficency = 0.125

		explosion_power = 1 //3,6,9,12? Or is that too small?
	...
	if(damage > warning_point) // while the core is still damaged and it's still worth noting its status
		if((world.timeofday - lastwarning) / 10 >= WARNING_DELAY)
			...
			if(damage > emergency_point)
				...
				message_admins("Got some noob engineers this round the Supermatter is about to blow! (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")

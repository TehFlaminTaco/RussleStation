	...
	if(status & ORGAN_DESTROYED)
		...
		switch(body_part)
			...
			if(HAND_RIGHT)
				...
				if(owner.gloves)
					if((istype(owner.gloves, /obj/item/clothing/gloves/assim))||(istype(owner.gloves, /obj/item/clothing/gloves/space_ninja)))
						del(owner.gloves)
					else
...
obj/item/weapon/organ/New(loc, mob/living/carbon/human/H)
	...
	if(base)
		if(H.species && (H.species.flags & HAS_COLOR))
			base.Blend(rgb(H.r_fur, H.g_fur, H.b_fur), ICON_ADD)
	...
	if(istype(W,/obj/item/weapon/scalpel))
		...
		switch(brain_op_stage)
			...
			if(3)
				...
				var/mob/living/simple_animal/borer/borer = src.has_brain_worms()
...
obj/item/weapon/organ/head/proc/has_brain_worms()

...
/obj/item/light_fixture_frame/proc/try_build(turf/on_wall)
	...
	switch(fixture_type)
		...
		if("emergency")
			newlight = new /obj/machinery/light_construct/emergency(constrloc)
...
/obj/machinery/light_construct/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/cable_coil))
		...
		switch(fixture_type)
			...
			if("emergency")
				src.icon_state = "bulb-constrcut-stage2"
		src.stage = 2
	...
	if(istype(W, /obj/item/weapon/screwdriver))
		if (src.stage == 2)
			...
			switch(fixture_type)
				...
				if("emergency")
					newlight = new /obj/machinery/light/emergency/built(src.loc)
...
/obj/machinery/light/emergency/built/New()
	status = LIGHT_EMPTY
	update(0)
	..()
	...
	if(istype(W, /obj/item/weapon/light))
		if(status != LIGHT_EMPTY)
			...
			if(istype(L, light_type))
				...
				if(istype(src, /obj/machinery/light/emergency))
					for(var/area/A in src.loc.loc)
	...
	// attempt to stick weapon into light socket
		...
		if(istype(W, /obj/item/weapon/screwdriver)) //If it's a screwdriver open it.
			...
			switch(fitting)
				...
				if("emergency")
					newlight = new /obj/machinery/light_construct/emergency(src.loc)
					newlight.icon_state ="bulb-constrcut-stage2"
			newlight.dir = src.dir
			newlight.stage = 2
			newlight.fingerprints = src.fingerprints
			newlight.fingerprintshidden = src.fingerprintshidden
...
/obj/item/light_fixture_frame/emergency
	name = "small emergency light fixture frame"
	desc = "Used for building small emergency lights."
	icon = 'icons/obj/lighting.dmi'
	icon_state = "bulb-construct-item"
	flags = FPRINT | TABLEPASS| CONDUCT
...
/obj/machinery/light_construct/emergency
	name = "small emergency light fixture frame"
	desc = "A small emergency light fixture under construction."
	icon = 'icons/obj/lighting.dmi'
	icon_state = "bulb-construct-stage1"
	anchored = 1
	layer = 5
...
/obj/machinery/light/emergency
	name = "emergency light fixture"
	icon = 'icons/obj/lighting.dmi'
	base_state = "firelight"		// base description and icon_state
	icon_state = "firelight1"
	desc = "A emergency lighting fixture."
	anchored = 1
	layer = 5  					// They were appearing under mobs which is a little weird - Ostaf
	use_power = 0
	idle_power_usage = 0
	active_power_usage = 0
	power_channel = null//Lights are calc'd via area so they dont need to be in the machine list
	on = 0					// 1 if on, 0 if off
	on_gs = 0
	brightness = 2			// luminosity when on, also used in power calculation
	status = LIGHT_OK		// LIGHT_OK, _EMPTY, _BURNED or _BROKEN
	flickering = 0
	light_type = /obj/item/weapon/light/emergency		// the type of light item
	fitting = "emergency"
...
/obj/item/weapon/light/emergency
...
/obj/machinery/light/emergency/power_change()
	spawn(10)
		var/area/A = src.loc.loc
		A = A.master
		if(!A.power_light && !A.fire && !A.eject)
			seton(!A.power_light)
		else if(A.fire)
			return
		else if(A.eject)
...
/obj/machinery/light/emergency/update(var/trigger = 1)
...
/obj/machinery/light/emergency/seton(var/s)
	on = (s && status == LIGHT_OK)
	update()


/obj/machinery/light/emergency/attack_hand(mob/user)

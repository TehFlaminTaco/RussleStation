...
/obj/machinery/recycler/proc/recycle(var/obj/item/I, var/sound = 1)
	...
	if(!istype(I, /obj/item/weapon/disk/nuclear))
		del(I)
		if(prob(15))
			new /obj/item/stack/sheet/metal(get_step(src, EAST))
		if(prob(10))
			new /obj/item/stack/sheet/glass(get_step(src, EAST))
		if(prob(2))
...
/obj/machinery/recycler/proc/stop(var/mob/living/L)
	...
	spawn(SAFETY_COOLDOWN)
		...
		safety_mode = 0
		update_icon()
/obj/machinery/recycler/proc/recyclefur(var/mob/living/simple_animal/L)
	L.loc = src.loc
	if(L.stat)
		playsound(src.loc, 'sound/effects/splat.ogg', 50, 1)
		if(prob(50))
			new /obj/item/stack/sheet/fur/SA(get_step(src, EAST))
			del(L)
		else if(L.meat_type)
			L.harvest()
		else
...
/obj/machinery/recycler/proc/eat(var/mob/living/L)
	...
	for(var/obj/item/I in L.get_equipped_items())
		if(L.unEquip(I))
			recycle(I, 0)
	if(isSA(L))
		var/mob/living/simple_animal/D = L
		if(prob(50))
			D.harvest()
			return
		else
